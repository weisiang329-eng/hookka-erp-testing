...
//           one physical piece.
function displayCoId(po: { poNo: string; itemCategory: string }): string {
...
export default function SalesOrderDetailPage() {
  ...
  // Phase 3 — opens the Service Order create modal pre-filled with this CO.
  ...
  // Put-On-Hold modal (0185). Holding a CO REQUIRES a non-empty reason; the
  // Confirm button stays disabled until one is typed and the backend rejects
  // blank. The reason rides to the production grid for CO-origin rows.
  ...
  // blank reason with a 400 (unified FE+BE rule). Omitted for other transitions.
  const updateStatus = useCallback(async (newStatus: SOStatus, holdReason?: string) => {
    ...
    try {
      res = await fetch(`/api/consignment-orders/${id}`, {
        method: "PUT",
  ...
  const confirmOrder = useCallback(async () => {
    ...
    let res: Response; let data: any;
    try {
      res = await fetch(`/api/consignment-orders/${id}/confirm`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleClone = () => {
    ...
    localStorage.setItem("co-clone-data", JSON.stringify(cloneData));
    navigate("/consignment/create?clone=1");
  };
  ...
  return (
    <div className="space-y-6 max-md:space-y-4">
      ...
      {/* Put-On-Hold Modal (0185) — a reason is REQUIRED. Confirm stays
          disabled until a non-empty reason is typed; the same rule is enforced
          on the backend. The reason flows to the production grid. */}
      {holdModal.open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="flex-1">
            ...
            {linkedPOs.length > 0 && (
              <p className="text-sm mt-1">
                Production Orders: {linkedPOs.map(displayCoId).join(", ")}
              </p>
            )}
          </div>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {/* Phase 3 — Service Order trigger. Same SHIPPED gate as on
                Sales Orders; backend re-validates server-side. CO has
                additional terminal states (PARTIALLY_SOLD/FULLY_SOLD) but
                those happen AFTER ship, so they qualify too. */}
            {["SHIPPED", "DELIVERED", "INVOICED", "CLOSED", "PARTIALLY_SOLD", "FULLY_SOLD"].includes(order.status) && (
              <Button
                variant="outline"
                size="sm"
                ...
                <tbody>
                  {linkedPOs.map((po) => (
                    <tr key={po.id} className="border-b border-[#E2DDD8] hover:bg-[#FAF9F7]">
                      <td className="px-3 py-3 doc-number font-medium">{displayCoId(po)}</td>
                ...
                {order.items.map((item) => {
                  ...
                  return (
                    <tr key={item.id} className="border-b border-[#E2DDD8] hover:bg-[#FAF9F7]">
                      ...
                      <td className="px-3 py-3 doc-number font-medium">{order.companyCOId}{item.lineSuffix}</td>
                      <td className="px-3 py-3">
      ...
      <DocumentFlowDiagram
        title={`Document Relationship — ${order.companyCOId}`}
        salesFlow={(() => {
          const nodes: DocNode[] = [
            {
              type: "SO",
              label: "Consignment Order",
              docNo: order.companyCOId,
              status: order.status,
              isCurrent: true,
              href: `/consignment/${order.id}`,
            },
          ];
          // If DO exists
