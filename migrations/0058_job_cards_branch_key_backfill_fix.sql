...
UPDATE job_cards SET branchKey = NULL;
UPDATE job_cards SET branchKey = '' WHERE departmentCode IN ('UPHOLSTERY','PACKING');
...
UPDATE job_cards SET branchKey = 'Foam'
  WHERE branchKey IS NULL
    AND departmentCode IN ('WOOD_CUT','FRAMING','WEBBING')
    AND (wipKey LIKE '%::DIVAN::%' OR wipKey LIKE '%::SOFA_BASE::%'
         OR wipKey LIKE '%::SOFA_CUSHION::%' OR wipKey LIKE '%::SOFA_ARMREST::%'
         OR wipKey LIKE '%::SOFA_HEADREST::%');

-- BF Headboard wood branch (top-of-subtree wipCode "Webbing").
UPDATE job_cards SET branchKey = 'Webbing'
  WHERE branchKey IS NULL
    AND departmentCode IN ('WOOD_CUT','FRAMING','WEBBING')
    AND wipKey LIKE '%::HEADBOARD::%';

-- BF Divan + Sofa fab branch (top-of-subtree wipCode "Fabric").
UPDATE job_cards SET branchKey = 'Fabric'
  WHERE branchKey IS NULL
    AND departmentCode IN ('FAB_CUT','FAB_SEW')
    AND (wipKey LIKE '%::DIVAN::%' OR wipKey LIKE '%::SOFA_BASE::%'
         OR wipKey LIKE '%::SOFA_CUSHION::%' OR wipKey LIKE '%::SOFA_ARMREST::%'
         OR wipKey LIKE '%::SOFA_HEADREST::%');
...
UPDATE job_cards SET branchKey = 'Foam'
  WHERE branchKey IS NULL
    AND departmentCode IN ('FAB_CUT','FAB_SEW','FOAM')
    AND wipKey LIKE '%::HEADBOARD::%';
...
UPDATE job_cards SET branchKey = 'Foam'
  WHERE branchKey IS NULL
    AND departmentCode = 'FOAM'
    AND (wipKey LIKE '%::SOFA_BASE::%' OR wipKey LIKE '%::SOFA_CUSHION::%'
         OR wipKey LIKE '%::SOFA_ARMREST::%' OR wipKey LIKE '%::SOFA_HEADREST::%');

UPDATE job_cards SET branchKey = '' WHERE branchKey IS NULL;
