...
| `src/pages/rack-scan.tsx` — rack QR stock-in (1097) | `src/api/routes/public-rack-qr.ts` — PUBLIC no-login rack stock-in + /p/ piece-sticker rack-write (auth-bypassed, idempotent) (711) | `kv_config` / `hookka_erp_metrics` | `tests/scan-per-piece.test.mjs` |
| | `src/api/lib/packing-rack-write.ts` — `applyPackingRack` (rack set/clear + `rack_items` occupancy mirror) | `rack_items` / `rack_locations` | `tests/packing-piece-identity.test.mjs` |
| | `src/api/lib/packing-piece-identity.ts` — `packingPieceIdentity` (shared /p/ + /r/ + office piece identity) | | |
