...
>
> **Rows 14, 15, 16 and 34 were FIXED on `fix/consignment-money-and-status`** and now say
> so inline.
>
> **Rows 12, 19 and 29 were re-opened on `fix/stock-grn-org-filter`** and each now records
> its outcome in place: 12 → **fixed** (BUG-2026-08-13-050), 29 → **fixed**
> (BUG-2026-08-13-051), 19 → **diagnosed and deliberately NOT fixed**
> (BUG-2026-08-13-052 — filling the column alone breaks the three-way-match panel; the
> reasoning is in that entry).
>
> Every other row is as originally reported and has NOT been re-checked since.
...
|---|---|---|---|---|
| 12 | ~~`src/pages/warehouse.tsx:610` `!po.stockedIn`~~ | `rowToMinimalPO` dropped it; full `rowToPO:978` has it | `production_orders.stocked_in`, genuinely written at `_helpers.ts:4933` | The Stock-In dropdown never excluded an already-stocked PO → duplicate rack assignment and a duplicate `STOCK_IN` movement. Highest-impact read on this list. **FIXED — BUG-2026-08-13-050.** It was a *regression*: the page read the full `rowToPO` shape until b7d00c78 (2026-05-23) slimmed seven consumers after grepping each for `.jobCards` only. Restored to the projection with an output-identity test |
...
| 14 | ~~`src/pages/consignment/detail.tsx:1093` `order.preHoldStatus`~~ | `rowToCO` now emits it; the PUT captures / preserves / clears `pre_hold_status` | `consignment_orders.pre_hold_status`, created by the runtime self-apply in `ensureDiscountColumn` | **✅ FIXED — BUG-2026-08-13-041.** Mirroring the SO fix exposed two more, both repaired with it: `VALID_TRANSITIONS.ON_HOLD` on **both** order types refused READY_TO_SHIP, so an order held there could not have been resumed at all once the true origin was stored; and both cascades defined a resume by its DESTINATION, so a resume to READY_TO_SHIP would have left the job cards ON_HOLD. COs already on hold have no stored origin and still resume to CONFIRMED — not backfilled |
| 15 | ~~`src/pages/consignment/edit.tsx:557` `expect: { … customerPOId … }`~~ | key removed from the `expect` map and from the request body | none — no column added | **✅ FIXED — BUG-2026-08-13-042.** The comparison was wrong, not the write. Pinned by `tests/verified-save-expect-contract.test.mjs`, which asserts the general invariant (*every `expect` key must be one the endpoint returns*) against the REAL by-id payload for the CO and against `rowToSO`'s emitted keys for the SO twin |
| 16 | `src/pages/consignment/detail.tsx` `order.hookkaDeliveryOrder` (`:1231`/`:1481`) | not emitted by `rowToCO` | none | ⬜ **still open.** The Document Relationship diagram never draws a DO node for any CO. The `customerPOId` half of this row is **✅ fixed** — the permanently-`-` "Customer PO" display is gone along with the two inputs that fed it (BUG-2026-08-13-042). The page is typed `SalesOrder`, which is why `tsc` was silent |
...
| 19 | *(server-internal)* `src/api/routes/grn.ts:1574` `poItem?.material_code \|\| poItem?.supplierSKU` | rows come from a raw `SELECT * FROM purchase_order_items`; the driver delivers `materialCode` / `supplierSku` | `grn_items.material_code` | Every PO-sourced GRN line is stored with `material_code = ""` → blank Material Code on GRN detail and the GRN PDF, and the binding-based RM resolver never fires. **Confirmed and still NOT fixed — BUG-2026-08-13-052**, which writes up *why* in full. Two blockers, not one: the RM resolver (`grn.ts:492-508`) keys the value as a **supplier SKU** while `fillGrnSupplierSku` keys it as an **internal code**, and `deriveMatCode`'s PO and GRN sides currently agree ONLY because both fall through to the same `materialName` split — fill the GRN side alone and the three-way-match panel on `procurement/detail.tsx:1398` reports every line as unmatched. Needs one coupled change plus an owner decision on which dimension the column is |
...
| 29 | ~~`src/pages/accounting/shared.ts:29` `value: o.code.toLowerCase()` → `&orgId=${company}`~~ | `src/api/lib/tenant.ts:213` binds that to the `org_id` column | `invoices`/`ledger_journal_entries`.`org_id` is `'hookka'` for **both** companies (`0142_organisations_registry.sql:92,100`) | The Accounting company selector cross-wired the *display* dimension (`code`, i.e. HOOKKA/OHANA — the real one is `sales_org_code`/`purchase_org_code`, `src/lib/company-dimension.ts:13`) onto the *tenant* column. **FIXED — BUG-2026-08-13-051.** Correction to the row's last sentence: "HOOKKA" did **not** work — it matched every row and printed the whole group's money under one company's name, and the Balance Sheet's per-company card showed RM 0.00 for all four. There is no mapping to substitute (neither `invoices` nor `ledger_journal_entries` carries a company column at all), so the selector now offers only the consolidated group option and hides itself |
...
| # | Site | What |
|---|---|---|
| 34 | ~~`src/pages/consignment/create.tsx:741`~~ | **✅ FIXED — BUG-2026-08-13-040.** All four priced components on the CO POST and PUT now go through the same resolvers the SO route uses, the unit price is the shared `calculateUnitPrice` naming all five parts, and `totalHeightPriceSen` is in both INSERT column lists (it was computed and then discarded). A **third** site turned up that the audit had not: `sofa-combo-pass.ts`, the recompute SHARED by both order types, rebuilt a renegotiated line from four components and a discount-free line total. Proof on the fixture, in sen: screen 98,500 unit / 194,000 line; stored BEFORE 90,500 / 178,000; stored AFTER 98,500 / 194,000. Blast radius on live data is **unmeasured** — run the row-34 probe in §7. Fix-forward; existing orders are not repriced |
