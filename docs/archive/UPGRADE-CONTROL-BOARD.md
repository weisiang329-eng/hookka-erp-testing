> **ARCHIVED / SUPERSEDED — stopped being true 2026-04-30** (the ETA on both of its "In Progress" rows, P7.4 and P4.3, never restamped since). It is NOT a single source of truth: it names files that do not exist in the tree — `src/api/lib/authz.ts` (the real RBAC lives in `src/api/lib/rbac.ts` + `role-policy.ts`), `src/api/routes-d1/mdm.ts` and `src/api/routes-d1/auth-oauth.ts` (no `routes-d1/` directory), and `docs/archive/UPGRADE-FINAL-READOUT.md` (moved to `docs/archive/`) — and its P7.4 gate figure is stale (measured 2026-08-13: `npx eslint .` = 20 errors / 97 warnings, not 92/15). Kept for history only; do not treat as current.
