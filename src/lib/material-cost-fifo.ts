...
// ---------------------------------------------------------------------------
// Periodic-inventory mode (owner rule 2026-07-03: "不要用 BOM 算先" — no
// BOM/FIFO auto-consumption until he opts back in; kv `rm_valuation_mode`).
//
// Stock value at a month-end = the latest stock-take count at or before that
// month, plus supplier-invoice purchases SINCE that count; before any count
// exists, the opening seed plus all purchases. Consumption between counts is
// therefore zero, and the month a count IS entered absorbs the true consumed
// amount (opening + purchases − counted closing) — the owner's periodic
// workflow. Pure so it unit-tests without the replay.
// ---------------------------------------------------------------------------
export function stockTakeChainValue(
  seedSen: number,
  piCumOf: (ym: string) => number, // cumulative PI purchases to ym (0 before the first month)
  stOf: (ym: string) => number | null, // stock-take value at ym, null when none
  ym: string,
  firstYm: string,
): number {
  let m = ym;
  let guard = 0;
  while (m >= firstYm && guard++ < 600) {
    const st = stOf(m);
    if (st !== null) return st + (piCumOf(ym) - piCumOf(m));
    m = prevYmStr(m);
  }
  return seedSen + piCumOf(ym);
}
