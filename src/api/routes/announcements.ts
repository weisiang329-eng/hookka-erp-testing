...
worker.get("/", async (c) => {
  ...
  if ((c.req.query("include") ?? "").toLowerCase() === "past") {
    // Archive = notices that have DROPPED OUT of the live list: hidden
    // (is_active=false) OR expired (expires_at in the past). Owner 2026-06-27:
    // an ACKED-but-still-active notice stays on the home banner (folded to its
    // title) — it does NOT come here until it expires. So acked-active rows are
    // deliberately NOT surfaced in the archive.
