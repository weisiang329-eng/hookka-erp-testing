...
// ---------------------------------------------------------------------------
// POST /api/raw-materials/merge-pairs  — ONE-TIME consolidation (owner 2026-06-18)
//
// Merge EXPLICIT duplicate pairs, keeping a chosen survivor (keepId) and
// repointing EVERY reference from the dropped row (dropId) onto it. Unlike
// /merge-duplicates (same item_code only), this handles CROSS-CODE merges
// where stock/receipts live on one code (e.g. "KS-01 BABY WHITE") while the
// sales/production references live on another ("KS-01"): we keep the short
// clean code and fold the stock + supplier prices + batches onto it.
//
// body: { apply?: boolean (default false = dry-run), pairs: [{ keepId, dropId }] }
//
// Per pair (drop -> keep), one ATOMIC batch:
//   id-refs:   rm_batches.rm_id, stock_adjustments.item_id (RM),
//              rd_material_issuances.raw_material_id, cost_ledger.item_id (RM)
//   code-refs (cross-code only): sales_order_items.fabricCode,
//              production_orders.fabricCode, supplier_material_bindings.materialCode
//   stock:     keep.balanceQty += drop.balanceQty (re-read fresh at apply time)
//   delete:    drop raw_materials row; cross-code fabric also drops the drop's
//              fabrics + fabric_trackings mirror (keep already owns its own).
//   Order matters: rm_batches.rm_id is ON DELETE CASCADE, so the repoint runs
//   BEFORE the delete inside the same atomic batch — the drop has 0 batches by
//   the time it is removed. The whole pair is all-or-nothing.
//
// Dry-run reports per-pair the live rows + every reference count on BOTH ids /
// codes so the operator can confirm the numbers before any write. This is a
// disposable tool — remove it + POST /_relock-duplicate-codes after the merge.
// ---------------------------------------------------------------------------
async function mergeRefCounts(
  DB: D1Database,
  id: string,
  code: string,
): Promise<{
  ...
  const n = async (sql: string, bind: string): Promise<number> => {
    const r = await DB.prepare(sql).bind(bind).first<{ n?: number | string }>();
    return Number(r?.n ?? 0);
  };
  const [
    batches,
    ...
    n("SELECT COUNT(*) AS n FROM rm_batches WHERE rm_id = ?", id),
    n("SELECT COUNT(*) AS n FROM stock_adjustments WHERE item_id = ? AND type = 'RM'", id),
    n("SELECT COUNT(*) AS n FROM rd_material_issuances WHERE raw_material_id = ?", id),
    n("SELECT COUNT(*) AS n FROM cost_ledger WHERE item_id = ? AND item_type = 'RM'", id),
    n("SELECT COUNT(*) AS n FROM sales_order_items WHERE fabricCode = ?", code),
    n("SELECT COUNT(*) AS n FROM production_orders WHERE fabricCode = ?", code),
    n("SELECT COUNT(*) AS n FROM supplier_material_bindings WHERE materialCode = ?", code),
    n("SELECT COUNT(*) AS n FROM fabrics WHERE code = ?", code),
    n("SELECT COUNT(*) AS n FROM fabric_trackings WHERE fabricCode = ?", code),
  ]);
  return {
    batches,
    adjustments,
...
app.post("/merge-pairs", async (c) => {
  const denied = await requirePermission(c, "raw-materials", "delete");
  if (denied) return denied;
  const body = (await c.req.json().catch(() => ({}))) as {
    apply?: boolean;
    pairs?: { keepId?: string; dropId?: string }[];
  };
  ...
  const loadRow = (id: string) =>
    c.var.DB.prepare(
      "SELECT id, itemCode, description, itemGroup, balanceQty, isActive FROM raw_materials WHERE id = ? AND orgId = ?",
    )
      .bind(id, orgId)
      .first<{
        id: string;
  ...
  for (const pr of pairs) {
    const keepId = pr.keepId ?? "";
    const dropId = pr.dropId ?? "";
    if (!keepId || !dropId || keepId === dropId) {
      plan.push({ keepId, dropId, error: "invalid pair (keepId/dropId missing or equal)" });
      continue;
    }
    const keep = await loadRow(keepId);
    const drop = await loadRow(dropId);
    if (!keep || !drop) {
      plan.push({ keepId, dropId, error: "keep or drop row not found in this org" });
      continue;
    }
    const [keepRefs, dropRefs] = await Promise.all([
      mergeRefCounts(c.var.DB, keep.id, keep.itemCode),
      mergeRefCounts(c.var.DB, drop.id, drop.itemCode),
    ]);
    plan.push({
      keep: {
        id: keep.id,
      ...
      drop: {
        ...
        qty: Number(drop.balanceQty),
        refs: dropRefs,
      },
      crossCode: keep.itemCode !== drop.itemCode,
      dropIsFabric: isFabricGroup(drop.itemGroup),
      resultingQty: Number(keep.balanceQty) + Number(drop.balanceQty),
    });
  }
  ...
  if (apply) {
    for (const p of plan) {
      if ((p as { error?: string }).error) {
        results.push({ ...p, applied: false });
        continue;
      }
      const keep = p.keep as { id: string; code: string };
      const drop = p.drop as { id: string; code: string };
      const crossCode = p.crossCode as boolean;
      const dropIsFabric = p.dropIsFabric as boolean;
      // Re-read the drop's stock fresh (the live ERP may have moved since the
      // plan was built) so the fold is exact.
      const dropNow = await loadRow(drop.id);
      const keepNow = await loadRow(keep.id);
      if (!dropNow || !keepNow) {
        results.push({ pair: [keep.code, drop.code], applied: false, error: "row vanished before apply" });
        continue;
      }
      const dropQty = Number(dropNow.balanceQty);

      const stmts: D1PreparedStatement[] = [
        // 1) repoint id-refs FIRST (rm_batches is ON DELETE CASCADE)
        c.var.DB.prepare("UPDATE rm_batches SET rm_id = ? WHERE rm_id = ?").bind(keep.id, drop.id),
        c.var.DB
          .prepare("UPDATE stock_adjustments SET item_id = ? WHERE item_id = ? AND type = 'RM'")
          .bind(keep.id, drop.id),
        c.var.DB
          .prepare("UPDATE rd_material_issuances SET raw_material_id = ? WHERE raw_material_id = ?")
          .bind(keep.id, drop.id),
        c.var.DB
          .prepare("UPDATE cost_ledger SET item_id = ? WHERE item_id = ? AND item_type = 'RM'")
          .bind(keep.id, drop.id),
      ];
      // 2) repoint code-refs only when the surviving code differs
      if (crossCode) {
        stmts.push(
          c.var.DB
            .prepare("UPDATE sales_order_items SET fabricCode = ? WHERE fabricCode = ?")
            .bind(keep.code, drop.code),
          c.var.DB
            .prepare("UPDATE production_orders SET fabricCode = ? WHERE fabricCode = ?")
            .bind(keep.code, drop.code),
          c.var.DB
            .prepare("UPDATE supplier_material_bindings SET materialCode = ? WHERE materialCode = ?")
            .bind(keep.code, drop.code),
        );
      }
      // 3) fold stock onto the survivor
      stmts.push(
        c.var.DB
          .prepare("UPDATE raw_materials SET balanceQty = balanceQty + ? WHERE id = ? AND orgId = ?")
          .bind(dropQty, keep.id, orgId),
      );
      // 4) delete the dropped row (now batch-free); cross-code fabric also drops
      //    the dropped code's fabrics + fabric_trackings mirror.
      stmts.push(
        c.var.DB.prepare("DELETE FROM raw_materials WHERE id = ? AND orgId = ?").bind(drop.id, orgId),
      );
      if (crossCode && dropIsFabric) {
        stmts.push(...buildFabricDeleteStatements(c.var.DB, drop.code));
      }

      await c.var.DB.batch(stmts);
      appliedCount++;
      results.push({
        pair: [keep.code, drop.code],
        applied: true,
        foldedQty: dropQty,
        crossCode,
        mirrorDropped: crossCode && dropIsFabric,
      });
    }
  }
  ...
  return c.json({
    ...
    data: {
      ...
      note: apply
        ? "Pairs merged — references repointed, stock folded, dropped rows + cross-code fabric mirrors removed."
        : "Dry-run only — review the counts, then pass { apply: true }.",
    },
  });
});
