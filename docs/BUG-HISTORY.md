...
**Fix (two passes):** First tried a dual-key read (`r.supplierSKU ?? r.suppliersku`) — deployed, but every PO was STILL blank on prod. So the value isn't merely mis-read: the create-time snapshot was never actually populated for ANY PO (the write path *looks* correct — `create.tsx` sends `supplierSKU: it.supplierSku`, backend binds it — but 0/161 landed, including a PO created that same day). Rather than keep chasing an unreproducible write gap, the real fix **derives the SKU on read from the (supplier, material) binding** — `supplier_materials`, which is 887/887 populated and IS the source of truth for a supplier's SKU. `loadSupplierSkuMap` + `fillBlankSupplierSku` in `purchase-orders.ts` fill any BLANK line SKU from the binding (keyed `supplierId + upper(materialCode)`); a non-blank stored value always wins. Wired into both the list (`GET /`, one query per page) and the single-PO `fetchPOWithItems` (covers detail + PDF). The earlier dual-key read is kept as harmless defense.

**Verified:** `tsc` clean. LIVE-VERIFY after deploy: PO-2607-008 shows "MED- COMFY IVORY SAND" in the Supplier SKU column, and the prod sweep of PO items flips from 0 to populated.
