...
-- migration number is stored).  Using ADD COLUMN IF NOT EXISTS preserves existing rows and
-- keeps older code (GRN cascade) working unchanged.
-- ============================================================================

-- --- raw_materials extensions -----------------------------------------------
ALTER TABLE raw_materials ADD COLUMN IF NOT EXISTS min_stock DOUBLE PRECISION NOT NULL DEFAULT 0;
ALTER TABLE raw_materials ADD COLUMN IF NOT EXISTS max_stock DOUBLE PRECISION NOT NULL DEFAULT 0;
ALTER TABLE raw_materials ADD COLUMN IF NOT EXISTS status TEXT NOT NULL DEFAULT 'ACTIVE';
ALTER TABLE raw_materials ADD COLUMN IF NOT EXISTS notes TEXT;
ALTER TABLE raw_materials ADD COLUMN IF NOT EXISTS created_at TEXT;
ALTER TABLE raw_materials ADD COLUMN IF NOT EXISTS updated_at TEXT;
...
ALTER TABLE rm_batches ADD COLUMN IF NOT EXISTS supplier_id TEXT;
ALTER TABLE rm_batches ADD COLUMN IF NOT EXISTS grn_id TEXT;
ALTER TABLE rm_batches ADD COLUMN IF NOT EXISTS total_value_sen INTEGER NOT NULL DEFAULT 0;
...
-- anything drops it).
ALTER TABLE fg_batches ADD COLUMN IF NOT EXISTS total_value_sen INTEGER NOT NULL DEFAULT 0;

-- --- Extra indexes the new routes rely on -----------------------------------
CREATE INDEX IF NOT EXISTS idx_rm_batches_supplier_id
  ON rm_batches(supplier_id);
CREATE INDEX IF NOT EXISTS idx_rm_batches_grn_id
  ON rm_batches(grn_id);
