...
## Entry points
- Pages
  - `/procurement` → `src/pages/procurement/index.tsx:801` (`ProcurementPage` — PO list, filters, grid; `POFormDialog` deep-link prefill at `:60`)
  - `/procurement/create` → `src/pages/procurement/create.tsx:70` (`CreatePurchaseOrderPage`; full-page PO create)
  - `/procurement/:id` → `src/pages/procurement/detail.tsx:94` (`PurchaseOrderDetailPage`; status actions + 412-requiresGrn guard; `ThreeWayMatchPanel` at `:1319`)
...
- API routes
  - PO CRUD + status lifecycle → `src/api/routes/purchase-orders.ts` (1016)
  - GRN CRUD + arrival + Post-to-Stock cascade → `src/api/routes/grn.ts` (1988)
  - PI CRUD + lifecycle + GL post → `src/api/routes/purchase-invoices.ts` (2110)
...
## Core flows
1. **Create PO** — `app.post("/")` `purchase-orders.ts:344`. `ensurePendingMigrations` at `:353`; takes `body.status` verbatim (MANUAL create sends `CONFIRMED`; defaults DRAFT only when omitted). Fills blank supplier SKUs from bindings (`fillBlankSupplierSku` `:171`).
2. **Receive → Post-to-Stock cascade** — GRN status derives from ARRIVAL on create (`grn.ts:1094`): local-in-hand (arrival ARRIVED) is born POSTED and posts immediately; OCR/import → DRAFT. Crossing to POSTED calls `postGRNToStock` (`:432`, resolves RM via `resolveRmForGRNItem` `:384`, bumps `raw_materials.balanceQty`, writes `cost_ledger`) then `cascadePOStatusAfterGRNPost` (`:722`, flips PO → RECEIVED/PARTIAL_RECEIVED). `COMMITTED_STATUSES = {CONFIRMED,POSTED}` (`:218`).
3. **Edit a POSTED GRN line** — `app.put("/:id")` `grn.ts:1459`; when prev+new both committed, `buildPostedGRNStockAdjustment` (`:581`) posts only the DELTA via the same helpers, and `cascadePOReceivedQtyDelta` (`:938`) moves the parent PO line. Blocked when `newAccepted < invoiced_qty` or lines added/removed (`checkGrnLineQtyEdit` in `purchase-edit-rules.ts:102`).
4. **PI create → convert-chain + GL post** — `app.post("/")` `purchase-invoices.ts:729`. `ensurePiMigrations` at `:736`; MANUAL → PENDING_APPROVAL, OCR (`ocrUsed`) → DRAFT. `checkConvertAvailability` (`convert-chain.ts:81`) line-level 409 guard; increments `grn_items.invoiced_qty`. Reaching APPROVED posts AP legs via `mapPurchaseLinesToAccounts` (`:148`) → `buildPiApprovalLegs` (`pi-posting.ts:35`).
5. **Edit PI (DRAFT or APPROVED)** — `app.put("/:id")` `purchase-invoices.ts:1458`, gated by `isPiEditable` (`purchase-edit-rules.ts:34`). Re-syncs `grn_items.invoiced_qty` (floored by `clampDecrement`, ceilinged by `checkInvoicedQtyCeilingAfterEdit` `:568`); an APPROVED edit posts a GL CORRECTION for the amount delta against a fresh sourceId.
6. **Supplier payment allocation** — `app.post("/")` `supplier-payments.ts:119` (multi-allocation MYR/FX; unallocated = advance); `/knock-off` (`:415`) / `/un-knock` (`:558`) reattribute an advance with NO GL move; void/unvoid via `buildSupplierPaymentLifecycle` (`:635`).

## Key functions / sections (locate-to-function)
| Symbol / section | file:line | Role |
|---|---|---|
| `ProcurementPage` | `src/pages/procurement/index.tsx:801` | PO list, filters, banner, grid |
| `POFormDialog` | `src/pages/procurement/index.tsx:60` | Create/edit PO modal (deep-link prefill) |
| `PurchaseOrderDetailPage` | `src/pages/procurement/detail.tsx:94` | PO detail; status actions, 412-requiresGrn guard |
| `ThreeWayMatchPanel` | `src/pages/procurement/detail.tsx:1319` | PO↔GRN↔PI variance panel (derived) |
| `app.post("/")` (PO create) | `src/api/routes/purchase-orders.ts:344` | PO create; `body.status` verbatim |
| `app.put("/:id")` (PO edit) | `src/api/routes/purchase-orders.ts:604` | PO edit + status lifecycle |
| `ensurePendingMigrations` (PO) | `src/api/routes/purchase-orders.ts:895` | Runtime column self-apply |
| `postGRNToStock` | `src/api/routes/grn.ts:432` | Post GRN lines to stock + cost_ledger |
| `cascadePOStatusAfterGRNPost` | `src/api/routes/grn.ts:722` | Flip parent PO → RECEIVED/PARTIAL_RECEIVED |
| `buildPostedGRNStockAdjustment` | `src/api/routes/grn.ts:581` | Compensating DELTA for POSTED-line edit |
| `cascadePOReceivedQtyDelta` | `src/api/routes/grn.ts:938` | Move PO line receivedQty by delta |
| `restorePOReceivedQtyForGRN` | `src/api/routes/grn.ts:834` | Un-post/cancel/delete: give back PO qty |
| `resolveRmForGRNItem` | `src/api/routes/grn.ts:384` | Resolve GRN line → raw_material |
| `app.post("/")` (GRN create) | `src/api/routes/grn.ts:1094` | GRN create; status derived from arrival |
| `app.put("/:id/arrival")` | `src/api/routes/grn.ts:1807` | Arrival state transition (gate) |
| `app.post("/")` (PI create) | `src/api/routes/purchase-invoices.ts:729` | PI create + convert-chain + GL post |
| `app.put("/:id")` (PI edit) | `src/api/routes/purchase-invoices.ts:1458` | PI edit (DRAFT+APPROVED) + GL correction |
| `checkInvoicedQtyCeilingAfterEdit` | `src/api/routes/purchase-invoices.ts:568` | Ceiling on re-synced invoiced_qty |
| `mapPurchaseLinesToAccounts` | `src/api/routes/purchase-invoices.ts:148` | PI lines → GL account buckets |
| `buildPiApprovalLegs` | `src/lib/pi-posting.ts:35` | PI AP GL legs on APPROVED |
| `isPiEditable` / `checkGrnLineQtyEdit` | `src/lib/purchase-edit-rules.ts:34 / 102` | Shared FE+BE edit gates |
| `checkConvertAvailability` / `clampDecrement` | `src/lib/convert-chain.ts:81 / 95` | Line-level 409 guard + floor |
| `app.get("/by-po/:poId")` | `src/api/routes/three-way-match.ts:221` | PO-scoped variance read |
| `app.post("/")` / `/knock-off` / `/un-knock` | `src/api/routes/supplier-payments.ts:119 / 415 / 558` | Pay PIs, apply/reverse advance |
| `buildSupplierPaymentLifecycle` | `src/api/routes/supplier-payments.ts:635` | Void/delete/unvoid shared core |
...
- **Three-Way-Match variance is derived**, not persisted (panel `detail.tsx:1319` + standalone `three-way-match.ts`). Don't store a second copy.
...
- **Change the receive/stock cascade** → edit `postGRNToStock` (`grn.ts:432`) + `cascadePOStatusAfterGRNPost` (`:722`); keep `restorePOReceivedQtyForGRN` (`:834`) symmetric. Verify `tests/grn-arrival-state.test.mjs` + `tests/purchase-edit-cascade.test.mjs`.
- **Adjust a PI GL posting** → change legs in `buildPiApprovalLegs` (`pi-posting.ts:35`) / bucket mapping `mapPurchaseLinesToAccounts` (`purchase-invoices.ts:148`); post on the APPROVED transition AND create-as-APPROVED; never mutate existing ledger legs.
...
- **Change supplier pricing** → binding CRUD in `supplier-materials.ts`; append audit via `price-history.ts`; render the log in `suppliers/detail.tsx:156`. Verify `tests/supplier-effective-pricing.test.mjs`.
