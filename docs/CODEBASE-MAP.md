...
| `src/pages/warehouse.tsx` — Grid / Stock In-Out / Movement History (1368) | `src/api/routes/qc-pending.ts` — generation runs 12:00/16:00 but on THREE rhythms: RM per goods receipt (`generateRmForGrns`), WIP per working dept (`stageHadActivity`), FG a sampled share of units produced (`generateFgSamples`); owns `completeInspection` (shared desktop+phone) + `ensureQcGenerationSchema` | `stock_movements` / `stock_adjustments` / `fg_units` / `grns` / `grn_items` | `tests/do-scan-sort.test.mjs` / `tests/qc-rm-families.test.mjs` |
| `src/pages/do-scan.tsx` — mobile DO sticker scanning | `src/api/routes/qc-templates.ts` — checklist templates (qc_templates + qc_template_items); RM templates carry `material_family` and POST refuses an RM template without one · `src/api/lib/qc-rm-families.ts` — item-group → material-family routing + `pickRmTemplate` | `fabric_trackings` / `audit_events` / `edit_presence` / `file_assets` | `tests/dept-scan-split.test.mjs` |
...
- **THREE STAGES, THREE RHYTHMS — do not "unify" them (owner 2026-08-08).** Generation still runs 12:00/16:00, but the *unit* differs per stage and that is the whole design. **RM = one inspection per GOODS RECEIPT** (`generateRmForGrns`), idempotent on `(source_grn_id, templateId)` — NOT on the slot, so the 16:00 run never re-raises the 12:00 receipt, and two GRNs on one day raise TWO inspections (the 2026-08-07 day gate collapsed them into one, i.e. a whole batch went uninspected, the opposite of "每一 batch 进来都必须抽检"). Lookback is 7 days on purpose: unbounded would raise thousands of slots for long-consumed goods on the first run after deploy. A **MIXED receipt raises one inspection PER MATERIAL FAMILY**, all carrying the same `source_grn_id` — deliberately not one blended checklist, because a single overall PASS/FAIL lets a fabric finding condemn the timber and the only valuable part of the record is *what* was wrong. **WIP = per working department per run**, unchanged. **FG = a sampled SHARE of units produced** (`fgSampleTarget`: 10%, floor 1, ceiling 5 per slot per category), each slot bound to ONE `fg_unit` with its sales-order line frozen into `so_spec` — "有没有做对 order" is answerable against a unit and an order, never against a template.
...
- **New QC columns are snake_case + runtime self-applied** (`ensureQcGenerationSchema`, awaited at the top of `generatePendingForSlot` and both `qc-templates` write paths): `qc_templates.material_family`, `qc_inspections.source_grn_id` / `source_grn_no` / `material_family` / `source_fg_unit_id` / `so_spec`. mig 0215 carries the same DDL plus the template CONTENT (every INSERT `ON CONFLICT DO NOTHING`, every UPDATE sets a literal — re-running is a no-op and an owner-edited item is never clobbered). **The migration file alone is inert** — 0215 must be run for the new templates to exist; until then `pickRmTemplate` falls back to the seeded ids so the four 0068 families keep working.
- **An RM or FG slot arrives with its subject ALREADY NAMED** — `quality.tsx` renders the receipt / unit banner (plus the `SoSpecPanel` for FG) instead of a dropdown, and skips the `/api/raw-materials` and `/api/fg-units` fetches entirely. Only WIP still asks the inspector to pick. Picking "some raw material" out of a list of every material in the company was never a record of anything.
