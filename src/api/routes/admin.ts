...
// ---------------------------------------------------------------------------
// POST /api/admin/fix-future-completion-dates  (owner data-tally audit 2026-07-11)
//
// One-shot repair: a bulk operation stamped FUTURE completion dates onto old
// completed production orders + all their job cards (e.g. an April order marked
// "completed" 2026-09-04). Because RM_ISSUE / FG_COMPLETED cost_ledger rows are
// dated by the PO's completedDate, fabric consumption + production output showed
// phantom quantities in future months (148.7m BF in 2026-09, 28m sofa 2026-12).
//
// Fix: for every PO whose completedDate is in the future, set completedDate (and
// its job_cards.completedDate, and its future-dated cost_ledger rows) to the PO's
// real last-touch date (updated_at), capped at today (owner ruling: use updatedAt).
// Only touches quantities' DATE, never the amounts. Idempotent — a re-run finds
// nothing once dates are past. ?dryRun=true (default) previews; execute with
// ?dryRun=false&confirm=YES_FIX_DATES. Delete this route post-repair.
// ---------------------------------------------------------------------------
app.post("/fix-future-completion-dates", async (c) => {
  const denied = await requirePermission(c, "users", "create");
  if (denied) return denied;
  const db = c.var.DB;
  const dryRun = (c.req.query("dryRun") ?? "true").toLowerCase() !== "false";
  const confirm = c.req.query("confirm") ?? "";
  if (!dryRun && confirm !== "YES_FIX_DATES") {
    return c.json(
      { success: false, error: "Pass ?dryRun=false&confirm=YES_FIX_DATES to execute." },
      400,
    );
  }
  ...
  const posRes = await db
    .prepare(
      `SELECT id, completedDate, updated_at FROM production_orders
        WHERE completedDate IS NOT NULL AND completedDate > ?`,
    )
    .bind(today)
    .all<Record<string, unknown>>();
  const plan = (posRes.results ?? []).map((r) => {
    const id = String(r.id);
    const from = String(r.completedDate ?? r.completed_date ?? "");
    let to = String(r.updated_at ?? r.updatedAt ?? "").slice(0, 10);
    if (!to || to > today) to = today; // updatedAt missing/future → cap at today
    return { id, from, to };
  });
  if (dryRun) {
    return c.json({ success: true, dryRun: true, count: plan.length, samples: plan.slice(0, 25) });
  }
  const cutoffIso = `${today}T23:59:59.999Z`;
  const stmts: D1PreparedStatement[] = [];
  for (const p of plan) {
    const iso = `${p.to}T12:00:00.000Z`; // matches the RM_ISSUE date format
    stmts.push(
      db.prepare("UPDATE production_orders SET completedDate = ? WHERE id = ?").bind(p.to, p.id),
      db
        .prepare(
          "UPDATE job_cards SET completedDate = ? WHERE productionOrderId = ? AND completedDate IS NOT NULL AND completedDate > ?",
        )
        .bind(p.to, p.id, today),
      db
        .prepare(
          "UPDATE cost_ledger SET date = ? WHERE refType = 'PRODUCTION_ORDER' AND refId = ? AND date > ?",
        )
        .bind(iso, p.id, cutoffIso),
    );
  }
  for (let i = 0; i < stmts.length; i += 60) {
    await db.batch(stmts.slice(i, i + 60));
  }
  return c.json({ success: true, dryRun: false, fixed: plan.length, samples: plan.slice(0, 25) });
});
