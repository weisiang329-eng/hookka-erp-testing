...
## Entry points
- Pages
  - `/reports` → `src/pages/reports.tsx:1855` (`ReportsPage` — tab router, `?tab=` URL sync)
...
- Report engines (logic lives here, not in the route)
  - `src/api/lib/compliance-report.ts` (1402) · `efficiency-report.ts` (644) · `schedule-overdue-report.ts` (713) · `operations-report.ts` (1223) · `production-brief.ts` (738)
...
## Core flows
1. **Daily Report (compliance)** — `GET /compliance.json` `reports.ts:601` → `buildComplianceCached` (`:653`, SWR snapshot cache, serve-stale + background refresh) → `collectComplianceData` (`compliance-report.ts:1304`) returns `ComplianceData` (`:314`: grouped exception rows + counts). `daily-report.tsx:967` renders it.
2. **Emailed report dispatch** — cron `POST /api/internal/reports/{efficiency,schedule,overdue,brief}-trigger` (`reports.ts:879/885/891/897`) → `cronGate` (`:854`, `authCron` shared-secret + skip Sun/PH) → `dispatchReport` (`:804`) resolves date + `resolveRecipients` (`:169`) → `runAndSendReport` (`:934`) collects data, renders HTML+text, `sendMail`.
3. **Manual send-now** — UI buttons hit `POST /efficiency/send` (`:374`), `/schedule/send` (`:918`), `/overdue/send` (`:923`), `/brief/send` (`:537`); all funnel into the same `dispatchReport` path but bypass the cron working-day gate.
4. **Production Brief** — `brief-trigger` (`:897`) is Agent-Console-gated (`isAgentPaused("PRODUCTION")`) and wrapped in `recordAgentRun` for token accounting; LLM budget via `llmKeyIfBudgetAllows`.
...
|---|---|---|
| `ReportsPage` | `src/pages/reports.tsx:1855` | Tab router + `?tab=` sync (default export) |
...
| `GET /compliance.json` | `src/api/routes/reports.ts:601` | Daily Report JSON (cached) |
| `buildComplianceCached` | `reports.ts:653` | SWR snapshot wrapper (serve-stale) |
| `collectComplianceData` | `src/api/lib/compliance-report.ts:1304` | Daily Report exception engine |
| `dispatchReport` | `reports.ts:804` | Shared send path (cron + Agent Console + manual) |
| `resolveRecipients` | `reports.ts:169` | kv_config → SUPER_ADMIN fallback |
| `cronGate` / `runAndSendReport` | `reports.ts:854 / 934` | Working-day gate · collect+render+send |
| `internal` (Hono) | `reports.ts:781` | `/api/internal/reports/*` cron triggers |
...
- **Logic lives in `src/api/lib/*`, not the route.** `reports.ts` (1016) is a thin shim over `compliance-report.ts` (1402), `efficiency-report.ts` (644), `schedule-overdue-report.ts` (713), `operations-report.ts` (1223), `production-brief.ts` (738). Edit the lib, not the handler.
...
- **Daily Report is expensive → snapshot-cached.** `collectComplianceData` cold-computes ~6s across the whole order/delivery/invoice/procurement chain; `buildComplianceCached` (`reports.ts:653`) serves last-good instantly and refreshes in the background. Numbers are byte-identical.
...
- **Change a Daily-Report exception rule** → edit the detector in `compliance-report.ts` and the `ComplianceData` shape (`:314`); the route + cache wrapper stay untouched. Grace windows are in `GraceDays` (`compliance-report.ts:60`).
- **Add / change an emailed report** → add a collect+render pair in a `src/api/lib/*-report.ts`, wire a `ReportKind` into `dispatchReport` (`reports.ts:804`) + a `-trigger` on the `internal` Hono (`:781`) + a manual `/send`. Recipients via `resolveRecipients` (`:169`).
- **Change report recipients / schedule** → `kv_config['daily_report_recipients']` (fallback SUPER_ADMINs); cron workflows drive the `-trigger` endpoints. Working-day skip logic is `cronGate`/`nonWorkingDayReason` (`reports.ts:854 / 154`).
