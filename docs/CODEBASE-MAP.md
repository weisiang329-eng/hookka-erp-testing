...
- **An invoice line's per-line enrichment resolves through `invoice_items.production_order_id`, NOT by product code** (`src/api/lib/invoice-print-extras.ts`, BUG-2026-07-17-001). Both halves: `refByPo` (`:290`) for the customer PO / SO / REF, and `priceByPo` (`:297`) for the Base/Divan/Leg/T.Height/Special build-up. `priceByPo` is built by scoping the sales-order item maps to the DO line's OWN sales order (`soIdByKey` `:146` resolves id/companySOId/companySO → the real SO id; `soTightH`/`soTight`/`soLoose`/`soByCode` `:189-192`) and matching on the production order's own spec incl. divan/leg/gap (`dims` `:70`). The old global `tight`/`loose`/`byCode` maps are **first-one-wins across every source SO** and survive ONLY as the fallback for lines with no PO link — on a consolidated DO they hand a line another variant's numbers, which is how an invoice printed "Base 0 … = RM 305" on a line the editor then re-summed to RM 308. Don't reintroduce a code-keyed price lookup ahead of the PO link.
- **A price build-up is displayed ONLY when it reconciles to the charged unit.** One rule, one function: `invoicePriceBreakdown` (`src/lib/build-unified-doc-data.ts:49`) returns undefined unless `base+divan+leg+totalHeight+special === priceSen`; the PDF and `src/pages/invoices/detail.tsx:746` both go through it. A non-reconciling line renders a single price. Never hand-roll a second breakdown renderer.
- The invoice editor's live sums (`detail.tsx:734` per-line Unit · `:395` header total · `:916` footer Subtotal) must include **all five** components — base, divan, leg, special AND `totalHeight`. The `DiscountInput` base at `:870` is `liveUnit * qty`, so an omission there silently shrinks a `%` discount too and the figure jumps after save (BUG-2026-07-17-001, 2026-08-07). Tests: `tests/invoice-price-by-po.test.mjs`.
