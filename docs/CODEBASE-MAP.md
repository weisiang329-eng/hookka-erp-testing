...
- Scan line → internal code resolves in THREE steps: supplierSku binding → supplierDescription binding → `src/lib/material-text-match.ts` matching the READ TEXT against **the materials THIS SUPPLIER is bound to** (`materialsForSupplier`), never the whole catalogue. Scoping is a correctness rule, not an optimisation: two boards differing only in size score almost identically on text, so choosing between them catalogue-wide is a guess about a real physical difference. A supplier with no bindings yields no candidates and the line is picked by hand — nothing has been bought from them, so there is nothing to recognise. The third step exists because the first appearance of an item has no binding to match, yet the OCR had plainly read the description — making the operator pick a line the scanner could identify itself. It is deliberately conservative (token Jaccard, MIN_SCORE 0.5, and the winner must beat the runner-up by MIN_MARGIN or it returns null): a blank line costs one pick, a WRONG match books stock and cost against the wrong material and surfaces much later. An auto-matched line is FLAGGED in the table so a guess never looks like an established mapping. Tests: `tests/material-text-match.test.mjs`.
