...
1. **D3** — it is a correctness bug on eight pages and the only one where the operator is
   told something false. Also the cheapest to prove: the `cancelled` flag already
   separates the two abort causes.
