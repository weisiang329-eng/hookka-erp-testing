...
CREATE INDEX IF NOT EXISTS idx_jc_po_dept
  ON job_cards(production_order_id, department_code);
...
CREATE INDEX IF NOT EXISTS idx_jc_po_wip_key
  ON job_cards(production_order_id, wip_key);
...
CREATE INDEX IF NOT EXISTS idx_prod_po_so_fabric
  ON production_orders(sales_order_id, fabric_code);
...
CREATE INDEX IF NOT EXISTS idx_cost_ledger_ref
  ON cost_ledger(ref_type, ref_id, type);
...
CREATE INDEX IF NOT EXISTS idx_fg_units_unit_serial
  ON fg_units(unit_serial);
