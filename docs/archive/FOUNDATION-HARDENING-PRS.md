> **ARCHIVED / SUPERSEDED — stopped being true once the seven `claude/sprint-*` branches merged (all sprint artifacts are on `main` as of 2026-08-13).** These are copy-paste PR bodies for branches that no longer need opening: `src/api/lib/rate-limit.ts`, `idempotency.ts`, `email-outbox.ts`, `src/lib/image-compress.ts`, `pricing-options.ts`, `csrf.ts`, `scripts/check-bundle-size.mjs`, `.github/workflows/backup.yml` and `process-email-outbox.yml` all exist. Three of its file references are now wrong: `migrations-postgres/0077_constraint_audit.sql` and `0078_org_id_full_rollout.sql` do not exist under those names (0077/0078 are `fg_units_cn_link` / `service_cases_affected_products` / `consignment_notes_in_transit`), `.github/workflows/refresh-mvs.yml` does not exist, and `src/api/routes-mock/worker-auth.ts` does not exist. Its final test plan also quotes `npm run lint:app` "~92 known errors" (measured 2026-08-13: 20 errors / 97 warnings) and "179+ tests" (there are now ~377 test files). Kept for history only; do not treat as current.
