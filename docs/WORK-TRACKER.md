...
## 2026-08-13 — 🔵 First real read-through of `accounting.ts` (13k lines, the money module) (branch `fix/accounting-audit`, **NOT deployed**)

The file had been UNGREPPABLE until `a9d413f6` — a raw NUL byte made GNU `grep` answer
`Binary file matches` and print nothing, so every grep-based audit had silently skipped
the whole money module. This pass read the code.

**Fixed (5, each with a regression test proved red by reintroducing the bug):**
1. ✅ **BUG-2026-08-13-080** — `/customer-statement` and `/supplier-statement` counted
   VOID/DELETED receipts/payments; their all-party ledger twins never did. The printed
   statement understated what a customer owes. New class **C18**.
2. ✅ **BUG-2026-08-13-081** — `POST /contra` flipped a PI to PAID without moving
   `paid_amount_sen`, without storing `bookedSen` (the only one of eight
   `supplier_payments` INSERTs in the repo that didn't) and without relieving
   `suppliers.outstandingSen`. AP counter drift that **Recalculate** could not clear.
3. ✅ **BUG-2026-08-13-082** — both AP subsidiary ledgers relieved the creditor by the
   CASH out of the bank (`amountSen`) instead of what came off `400-0000` (`bookedSen`).
   FX payments only; not measured against prod rows.
4. ✅ **BUG-2026-08-13-083** — manual journals had no tenant boundary at all, justified
   by an in-file comment whose premise (`journal_entries has no orgId column`) is false.
   Closes **C12 row 11**. Byte-identical on one tenant.
5. ✅ **BUG-2026-08-13-084** — five MORE files still carried a raw NUL byte, two on the
   money path (`ap-recon.ts`, `pnl-historical.ts`) plus this very file. New class **C17**
   + a repo-wide guard.

**🟡 Owner decisions collected, deliberately NOT decided** — see the PR body: what a
contra should look like on a printed customer/supplier statement; whether
`POST /landed-cost` needs an idempotency key; whether `PUT /journals/:id` should refuse
an all-zero draft edit; whether `nextJeNo` should number per tenant.

Gates: `npx tsc -p tsconfig.app.json --noEmit` exit 0 · `npm test` 3,857/0 fail ·
`check-codebase-map` OK · `check-docs-freshness` OK. **Not deployed, not merged** — the
main session deploys and verifies on prod.

## 2026-08-13 — 🔵 Three layer-audit rows: stockedIn / GRN material_code / accounting org filter (branch `fix/stock-grn-org-filter`, PR #304, **NOT deployed**)

The three rows the layer-consistency audit reported but did not fix
(`docs/AUDIT-LAYER-CONSISTENCY.md` rows 12, 19, 29). Asks, in the order given:

1. ✅ **`stockedIn` missing from the minimal PO projection** → BUG-2026-08-13-050.
   Restored to `rowToMinimalPO` / `MinimalPOOut`. Confirmed first that it is a real
   flag with two live writers and a ledger consequence (not the `actualMinutes` trap),
   and that it is a **regression** from b7d00c78 (2026-05-23), whose per-consumer audit
   grepped for `.jobCards` only. Guard `tests/minimal-po-stocked-in.test.mjs` proves
   output identity — the projection's whole key set plus value-for-value agreement with
   `rowToPO` — and was seen red.
2. ✅ **Sweep every other consumer of the minimal projection** → four more same-class
   reads found and enumerated as new BUG-CLASS **C16**; all display-only, all left
   alone with a written reason.
3. 🟡 **`grn_items.material_code` written as `""`** → BUG-2026-08-13-052,
   **diagnosed, deliberately NOT fixed, owner decision owed.** The value IS knowable at
   write time (both operands of `grn.ts:1574` are dead driver keys — the rename map has
   two entries for `supplier_sku` and `Object.fromEntries` keeps the last), but filling
   it alone breaks the three-way-match panel on `procurement/detail.tsx:1398` — the PO
   and GRN sides of `deriveMatCode` agree today only because both fall through to the
   same `materialName` split — and it changes which raw material `resolveRmForGRNItem`
   posts stock against. Needs one coupled change plus a decision on what the column
   means (an internal code to `fillGrnSupplierSku`, a supplier SKU to the RM resolver).
   **No history touched**; probes are in the bug entry.
4. ✅ **Accounting company selector filtering `org_id` with a display code** →
   BUG-2026-08-13-051. No mapping invented: neither `ledger_journal_entries` nor
   `invoices` carries a company column, so a per-company P&L cannot be derived at all.
   Selector collapsed to the consolidated group option and hidden. Correction to the
   audit: "HOOKKA" did not work either — it printed the whole group's money under one
   company's name, and the Balance Sheet's per-company card showed RM 0.00 for all four.

Gates: `tsc -p tsconfig.app.json --noEmit` exit 0 · `npm test` 3,864 tests / 0 fail ·
`eslint` on every changed file 0 errors. Every behavioural guard proved by reintroducing
the bug. **Not deployed, no prod session** — deploy note for `-050`: the
`?fields=minimal&include=` payload is snapshot- and KV-cached with no schema version in
either key, so wipe `production_orders_list_snapshot` after deploying or the old shape
keeps being served.

---

## 2026-08-13 — 🔵 超时不得报成「查无此单」（branch `fix/timeout-not-notfound`，PR 待合并，**未部署**）

BUG-2026-08-13-016 / 审计 D3 / 新类别 **C15**。`api-client.ts` 在 30 秒 abort 每个
`/api/*`；`cached-fetch.ts` 的 hook 以前 `if (isAbortError(err)) return;` **不设 error**，
`.finally` 又把 loading 清掉 —— 消费者拿到 `data=null, loading=false, error=null`，
与「服务器答了、这里什么都没有」**逐字节相同**。于是 11 个页面把死掉的请求印成
**"Order not found" / "Invoice not found" / "GRN not found."**，4 个页面（含
`sales/edit`、`consignment/edit` 两个审计漏掉的）永远停在 **"Loading…"**，公开扫码页
`track` 还给送货师傅打出 **"Unit not found"**。「查无此单」是**对生意的陈述**，操作员会
照做——重新开一张单。

**做法**：`joinInflight` 改抛带 status 的 `HttpError`（message 一字未改）；
`classifyFetchFailure` 给出 `{kind, status, message}`，`kind` 只有 **HTTP 404** 才是
`notFound`；`isUnknownOutcome(failure)` 是唯一守卫；`useCachedJson` 新增 `failure`；
新增 `<RecordLoadError>`（reports 那张 `<ReportError>` 的记录级版本：原因 + Retry +
「这不代表这张单不存在」）。**30 秒上限没动，写路径没加重试**；**取消（unmount / 换
URL）仍然静默**——`cancelled` 守卫仍是 catch 的第一行。

改了 19 个文件，另有 6 个（4 个 `/m` 画面 + `mail-center/detail`、`mail-center/index`）
本来就把 "not found" 挡在 hook 的 `error` 后面，被底层修复顺带治好，已列入类别表跟踪
（不是豁免）。

**仍未修**：同一类别的**列表页**（约 25 个 grid 在失败的 fetch 上印 "No draft orders."
之类）＝ C15 第 3 行，另开 PR；每页有自己的 empty caption，盲扫不安全。

门禁：`tsc` 干净、`npm test` 3810 全过（新增 33 条）、`eslint` 0 error（`suppliers/detail`
两条 warning 在 `main` 上同样存在）。五个守卫都用「把 bug 放回去看它变红」验证过——其中
一条**第一次没红**（只查了标识符是否出现，而守卫已被短路出分支），已改成要求那次调用
真正 gate 住 `<RecordLoadError>`。**未部署、未在浏览器/prod 验证**，交主 session。

---

## 2026-08-13 — 🔵 两个独立 perf 修复（branch `perf/mobile-home-and-pdf-chunk`，PR 待合并，**未部署**）

1. **`/m` Home 的 Pending Delivery 改读 `/api/delivery-orders/pending-value`**
   （BUG-2026-08-13-011）。手机端早就不再从 `/api/production-orders` 推导了，但仍拉
   整份 `/ready-planning` 的 `{ready[], planning[]}` 只为在浏览器里 reduce 一个
   `Σ valueSen`。工厂现场用手机、网络更差，风险比桌面更大不是更小。数字含义未变：
   仍是 server ready 合计 + DRAFT/LOADED/IN_TRANSIT dispatch chain，与桌面 KTile
   同一套折算。已扫过 `src/pages/m/` 其余画面，没有第二处同型。
2. **`__vitePreload` 从 `pdf` chunk 里搬出来**（BUG-2026-08-13-012）。它是虚拟模块
   `vite/preload-helper.js`，旧的 `manualChunks` 开头 `if (!id.includes('node_modules')) return`
   直接跳过它，rolldown 于是把它塞进 1,036 KB 的 pdf vendor chunk —— 结果**任何含
   `await import(...)` 的 chunk 都静态依赖那 1 MB**，实测 53 个。改用
   `codeSplitting.groups` + `test: /preload-helper/`（`manualChunks` 那条兼容路径会
   **静默忽略**这个模块的分组，已验证）。实测 `/employees` 静态闭包
   1,997 KB → 962 KB（gzip 574 KB → 271 KB）；pdf chunk 静态引用者 53 → 14，全是真正
   的 PDF 消费者；`import()` 图前后完全一致（273 个目标、0 缺失、50 个指向 PDF chunk）。

**注意**：`node scripts/check-bundle-size.mjs` 在 `main` 上本来就 FAIL（baseline 停在
2026-08-11，`reports` / `verified-save` / `cached-fetch` / `BUG-HISTORY` /
`finance-dashboard` 五个 stem 漂移）。本分支前后跑出来是**同样五个、同样数字**，没有新
增回归；唯一新 stem 是 `vite-preload` 1.17 KB（属信息项，不判失败）。**没有**跑
`--write-baseline`：`refresh-bundle-baseline.yml` 每次 push 到 main 会自动重生成，在分支
上重生成只会把漂移埋掉。

部署与 prod 验证由主 session 负责，本分支未部署、未在浏览器里跑过。
...
1. **RM：同一天验一次，不是每张 GR 验一次。**「同一天的话，通常是验一次就够了」。
   56abaaf0/29aa30f7 出的是每张 GRN 一张 inspection，同一天两张 GR 就两张。要合成一张，
   但**每张 GR 都要记在那张 inspection 上**，验的人才知道眼前有哪几批货。而且**当天晚点
   再来一张 GR，要挂到已经开着的那张**，不是另开一张。判断题：同一天、同一 family、
   **不同供应商**算一张还是两张。
2. **FG：抽样要按风险加权，不是 10% 乱抽。** 要偏向 (a) 少出现的 order（要从数得出来的
   东西定义「稀有」，不是手维护名单）、(b) 看手工的款、(c) **最强讯号：service case
