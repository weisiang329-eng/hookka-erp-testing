...
CREATE TABLE working_hour_entries (
  ...
  hours REAL NOT NULL DEFAULT 0,       -- decimal hours; UI inputs e.g. 7.5h directly
  notes TEXT,
  createdAt TEXT NOT NULL DEFAULT (datetime('now')),
  updatedAt TEXT NOT NULL DEFAULT (datetime('now')),
  FOREIGN KEY (attendanceId) REFERENCES attendance_records(id) ON DELETE CASCADE,
  FOREIGN KEY (workerId) REFERENCES workers(id) ON DELETE CASCADE
);

CREATE INDEX IF NOT EXISTS idx_working_hour_entries_attendance
  ON working_hour_entries(attendanceId);

CREATE INDEX IF NOT EXISTS idx_working_hour_entries_worker_date
  ON working_hour_entries(workerId, date);

CREATE INDEX IF NOT EXISTS idx_working_hour_entries_date_dept
  ON working_hour_entries(date, departmentCode);
...
-- ----------------------------------------------------------------------------
INSERT OR IGNORE INTO departments (id, code, name, shortName, sequence, color, workingHoursPerDay) VALUES
  ...
  ('dept-12', 'PRODUCTION_SHORTFALL', 'Production Shortfall', 'Shortfall', 12, '#DC2626', 9);

-- ----------------------------------------------------------------------------
-- workers.otMultiplier — per-worker OT premium. Default 1.5× hourly rate.
-- ----------------------------------------------------------------------------
ALTER TABLE workers ADD COLUMN otMultiplier REAL NOT NULL DEFAULT 1.5;
