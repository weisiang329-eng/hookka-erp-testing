...
| `job_cards.production_time_minutes` | `= est_minutes` | `bom.ts:1055-1063` binds **the same `p.target`** to both columns; `src/lib/production-order-builder.ts:183/186` binds the same `p.minutes` to both at card creation; migrations `0027`/`0029` write both from one literal `CASE` |
