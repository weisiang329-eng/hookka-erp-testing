...
- `dueFrom` / `dueTo` do **NOT** filter `targetEndDate`, whatever the comment above them at `src/api/routes/production-orders.ts:805` says (it still reads "Server-side targetEndDate window"; the vars are at `:809`). Since 2026-06-10 the OVERVIEW branch (no `dept`) windows the PO's SO `hookkaExpectedDD` (`ddExpr` in `_helpers.ts`) and the DEPT branch windows that dept's `job_cards.dueDate`. Undated rows pass on both sides deliberately. Also note there is no far-future tail to trim on this dataset — every active PO's expected DD sits inside ~7 months, so a `dueTo` horizon removes ~0.2 % of rows and is not a perf lever (measured 2026-08-13).
...
- **`src/api/lib/sequence-lock.ts` — one answer to "may this job card move yet?" (2026-09-07).** Pure: no DB, no department table. `sequenceBlockers(card, allCards)` returns the cards that must finish first, read off the BOM's own shape — a card waits for every earlier-`sequence` card in its own `(wipKey, branchKey)`, and a card whose `branchKey` is EMPTY (the BOM's mark for a step belonging to the whole product) also waits for the LAST card of every other branch in that `wipKey`. Change a BOM and this follows; there is nothing to maintain. It landed on main AHEAD of the gate that refuses with it, because `scripts/backfill-skipped-completions.mjs` has to close exactly the cases the gate rejects — **on main nothing in the API imports it, so it refuses nothing today**; the gate arrives with the sequence-lock PR. Never read `PRODUCTION_ORDER_BY_WIP_TYPE` from here (build-time ordering) and never `job_cards.prerequisiteMet` (stamped once at build, never rolled forward — measured 2026-09-06: at least 2,611 of 4,967 cards on active orders read it wrongly). Tests: `tests/sequence-rule-unit.test.mjs`.
...
|---|---|---|---|---|
| `src/api/lib/sequence-lock.ts` | 170 | The ONE answer to "may this job card move yet?" — a job card waits for every earlier-`sequence` card in its own `(wipKey, branchKey)`, and a card whose `branchKey` is EMPTY (the BOM's marker for a step belonging to the whole product, set by `bom-wip-breakdown.ts` when it walks the tree) additionally waits for the LAST card of every other branch. **Pure — it reads job cards and nothing else.** No table, no constant: the sequence and branch were stamped from the BOM at build time, so changing a BOM changes the lock for the next cards built. It must never import `DEPT_ORDER` / `PRODUCTION_ORDER_BY_WIP_TYPE` (a test forbids it) — that flat order lists WOOD_CUT after FAB_SEW and would block wood cutting on fabric sewing. It must also never read `job_cards.prerequisiteMet`, which is stamped once at creation and never rolled forward (measured 2026-09-06: 4,680 of 4,967 active cards read 0, at least 2,611 falsely). Callers: `applyPoUpdate` (covers the grid AND bulk-patch, which loops back into it), `POST /:id/scan-complete`, and both fan-out scans via `gateFanOutSequence`. Refusal is `409 { code: "UPSTREAM_INCOMPLETE", blockedBy, canSelfUnlock }`; `canSelfUnlock` is the server's decision, never the client's. | 0 (pure lib) | `tests/sequence-lock.test.mjs` |
...
| `src/api/routes/import-completion/price-backfill.ts` | 836 | The July/August repricing writes that the ordinary edit paths cannot reach: `refresh-so-surcharges` re-derives divan/leg/total-height/special from the owner's CURRENT `variants-config` lists (the resolvers derive only when the field is OMITTED, so it passes `undefined` on purpose — passing the stored value back is a no-op with extra steps). **Dry-run by DEFAULT** (`?dryRun=false` writes). Service orders are excluded with NO flag to override — 「0就是0 有amount就是有amount」. `backfill-invoice-prices-from-so` re-derives invoice lines from their sales-order lines and applies them by SELF-CALLING `PUT /api/invoices/:id` — the GL restatement (reverse + re-post on one hash chain) stays in `invoices.ts` and is never copied here, which a test enforces. Leaves a hand-set price alone but re-derives a hand-set ZERO (the BUG-2026-08-20-158 damage). | 2 | `tests/refresh-so-surcharges.test.mjs`, `tests/invoice-backfill-from-so.test.mjs` |
| `src/api/routes/import-completion/supplier-price-refresh.ts` | 262 | Brings `supplier_material_bindings` up to what we ACTUALLY paid, because every PO autofills its unit price from there — correcting historical invoices and leaving the list stale fixes the past and keeps mispricing the future (measured 2026-08-28: OCEAN SKY's three fastener lines stamped 2026-05-05 while August invoices paid differently on all three, two up one down). Source is the most recent PURCHASE INVOICE line for that supplier+material — **not** the scan (unreviewed), the PO (what we asked for, not what we were charged), or an average (nobody agreed to an average). `effectiveFrom` is copied from that invoice's date, never today. Refuses: two prices on the same latest date, a list already dated NEWER than the last invoice (that is a decision, not staleness), zero-priced lines, cancelled invoices. A never-invoiced material is passed over, not zeroed. The write SELF-CALLS `PUT /api/supplier-materials/:id` so `price_histories` keeps exactly one writer. `GET /supplier-price-drift` writes nothing; `POST /refresh-supplier-price-list` is **dry-run by DEFAULT** and scopeable with `?supplierId=`. | 2 | `tests/supplier-price-refresh.test.mjs` |
| `src/api/routes/import-completion/rounded-price-repair.ts` | 441 | Recovers the sub-cent unit prices an INTEGER `unit_price_sen` rounded away before 2026-08-28. The rounding happened on the way IN and the line total was recomputed from the rounded rate, so the invoice agrees with itself and nothing in the ERP disagrees — no arithmetic recovers it. The evidence is `scan_queue.raw_json`, the scanner's structured reading of the supplier's PDF (bytes are NULLed on consume; that JSON is not), so the repair COPIES the supplier's number and never derives one. A line is eligible only if the scanned document agrees with ITSELF (`qty x unitPrice` = the `amount` read off the same row, within a sen) AND the gap is under one sen — a bigger gap is a renegotiated price, not this bug. Ambiguous pairings are refused, never picked. PAID/PARTIAL_PAID excluded in the WHERE, not downstream. The header is RE-DERIVED from its lines. **Ledger legs are NOT rewritten here** — a posted invoice goes back through Edit so the GL restatement runs on the hash chain; the response names which. Hand-typed POs are out of scope (no scan = no evidence) and are counted rather than guessed at. `GET /rounded-unit-prices` writes nothing; `POST /repair-rounded-unit-prices` is **dry-run by DEFAULT**. | 2 | `tests/rounded-price-repair.test.mjs` |
