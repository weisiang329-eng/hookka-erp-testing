...
ALTER TABLE users ADD COLUMN roleId TEXT REFERENCES roles(id);
...
INSERT OR IGNORE INTO roles (id, name, description) VALUES
  ('role_super_admin', 'SUPER_ADMIN',  'Full system access; manages users, roles, permissions, audit log'),
...
INSERT OR IGNORE INTO permissions (id, resource, action, description) VALUES
  ...
  ('perm_consignment_notes_delete', 'consignment-notes', 'delete', 'Delete consignment notes');
...
INSERT OR IGNORE INTO permissions (id, resource, action, description) VALUES
  ...
  ('perm_three_way_match_delete', 'three-way-match', 'delete', 'Delete 3-way match records');
...
INSERT OR IGNORE INTO permissions (id, resource, action, description) VALUES
  ...
  ('perm_rm_batches_delete', 'rm-batches', 'delete', 'Delete RM batches');
...
INSERT OR IGNORE INTO permissions (id, resource, action, description) VALUES
  ...
  ('perm_forecasts_delete', 'forecasts', 'delete', 'Delete forecasts');
...
INSERT OR IGNORE INTO permissions (id, resource, action, description) VALUES
  ...
  ('perm_fg_units_delete', 'fg-units', 'delete', 'Delete finished-goods units');
...
INSERT OR IGNORE INTO role_permissions (roleId, permissionId)
SELECT 'role_read_only', id FROM permissions WHERE action = 'read';
...
SELECT 'role_finance', id FROM permissions
WHERE action = 'read'
  AND resource IN ('sales-orders', 'purchase-orders', 'customers', 'suppliers');
...
SELECT 'role_procurement', id FROM permissions
WHERE action = 'read'
  AND resource IN ('production-orders', 'bom', 'fabrics');
...
SELECT 'role_production', id FROM permissions
WHERE action = 'read'
  AND resource IN ('sales-orders', 'products', 'raw-materials', 'inventory');
...
SELECT 'role_warehouse', id FROM permissions
WHERE action = 'read'
  AND resource IN ('production-orders', 'purchase-orders', 'grn');
...
SELECT 'role_sales', id FROM permissions
WHERE action = 'read'
  AND resource IN ('production-orders', 'products', 'inventory', 'fg-units');
...
WHERE action = 'read'
  AND resource IN ('payslips', 'attendance', 'production-orders', 'job-cards');
...
UPDATE users SET roleId = 'role_super_admin' WHERE role = 'SUPER_ADMIN' AND roleId IS NULL;
UPDATE users SET roleId = 'role_worker'      WHERE role = 'WORKER'      AND roleId IS NULL;
-- STAFF defaults to read_only until per-user assignment.
UPDATE users SET roleId = 'role_read_only'   WHERE role = 'STAFF'       AND roleId IS NULL;
-- Anyone else (unknown role) defaults to read_only.
UPDATE users SET roleId = 'role_read_only'   WHERE roleId IS NULL;
