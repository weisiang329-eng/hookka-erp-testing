...
ALTER TABLE departments ADD COLUMN isProduction INTEGER NOT NULL DEFAULT 1;

-- Mark existing depts: 8 production = 1; 4 non-production = 0.
UPDATE departments SET isProduction = 0 WHERE code IN
  ('WAREHOUSING','REPAIR','MAINTENANCE','PRODUCTION_SHORTFALL');

-- New R&D dept (non-production, sequence 13).
INSERT OR IGNORE INTO departments (id, code, name, shortName, sequence, color, workingHoursPerDay, isProduction)
VALUES ('dept-13', 'R_AND_D', 'R&D', 'R&D', 13, '#0EA5E9', 9, 0);
