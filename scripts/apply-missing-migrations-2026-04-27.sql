...
--
-- Concatenated catch-up bundle for the 9 tables flagged by the schema-diff
-- agent in commit bfb7c2c (CI post-deploy check vs live Supabase Postgres).
-- These migrations exist in migrations-postgres/ but were never applied to
-- the live database, so runtime hits "relation does not exist" the moment
-- any of these 9 tables is touched.
--
-- Generated: 2026-04-27
-- Source files (in apply order, original migration sequence):
--   0045_rbac.sql               -> roles, permissions, role_permissions
--   0046_audit_events.sql       -> audit_events
--   0048_worker_sessions.sql    -> worker_sessions
--   0051_journal_entries.sql    -> ledger_journal_entries
--   0052_mdm_review_queue.sql   -> mdm_review_queue
--   0053_oauth_identities.sql   -> oauth_identities
--   0055_file_assets.sql        -> file_assets
--
-- Apply: paste this whole file into Supabase SQL Editor and run.
-- All CREATE TABLE / CREATE INDEX use IF NOT EXISTS, all INSERTs use
-- ON CONFLICT DO NOTHING, so the bundle is idempotent and safe to re-run.
--
-- Dependency note: 0045 ALTER TABLE users ADD COLUMN role_id REFERENCES
-- roles(id). This requires the users table (from 0002_auth.sql) to exist
-- in the live DB. It already does — the schema-diff agent only flagged
-- the 9 tables listed above as missing. No other cross-migration deps.
--
-- If a single block fails, the -- === <filename> === comments below are
-- the split points: copy from one === marker to the next and re-run.
-- ===========================================================================
...
CREATE TABLE IF NOT EXISTS mdm_review_queue (
  id TEXT PRIMARY KEY,
  resource_type TEXT NOT NULL,        -- 'customers', 'suppliers', 'products'
