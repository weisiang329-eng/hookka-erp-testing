...
> **Last verified: 2026-08-19** against `src/pages/products/{index,bom,documents,catalog}.tsx`,
> `src/api/routes/{products,customer-products,bom,bom-master-templates,product-configs,maintenance-config,mdm}.ts`,
> `src/api/routes/files.ts`, `src/api/lib/column-rename-map.json`, `tests/db-schema.json`.
> Corrected 2026-08-19: **every page-side anchor was 3–5 lines stale** and is re-derived —
> `ProductsPage` `:2004`→`:2007`, `VariantEditorDialog` `:651`→`:654`, `MaintenanceView`
> `:1109`→`:1112`, `CustomerAssignmentsSection` `:469`→`:472`, `ProductionConfig`/`CategoryBadge`
> `:376`/`:363`→`:379`/`:366`, `BOMPage` `:457`→`:462`, `ProductDocumentsPage` `:87`→`:92`.
> Also: the master-template bulk replace is `app.put("/")` at `:190` (was `:189`); the
> `masterPending` field is really `masterPendingEffectiveFrom` and is built at `:184-206`
> (was `:189`); the `/api/files` disposition sentence described the wrong handler. The
> stamp's own "5,307 lines" was wrong — `index.tsx` is **5,316**, which is what the Gotchas
> section already said. Every `products.ts`, `bom.ts`, `maintenance-config.ts`, `mdm.ts` and
> `customer-products.ts` anchor re-verified **exact**; `ProductCatalog :138` exact.
...
- **Add a field to a product** → column self-apply near `ensureProductCreatedAtColumn` (`products.ts:28`); persist in `app.post("/")` (`:584`) + `app.put("/:id")` (`:730`); surface in `rowToProduct` (`:161`); render in `VariantEditorDialog` (`index.tsx:654`). New column = snake_case (+ rename-map if camelCase). Verify BOM math with `tests/bom-explosion.test.mjs`.
