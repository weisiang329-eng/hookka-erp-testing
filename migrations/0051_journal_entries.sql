...
CREATE TABLE IF NOT EXISTS ledger_journal_entries (
  ...
  sourceType TEXT NOT NULL,        -- 'invoice', 'payment', 'credit_note', 'debit_note', 'manual'
  sourceId TEXT NOT NULL,          -- the row PK in its source table
  -- The double-entry pair (one row per leg).
  legNo INTEGER NOT NULL,          -- 1, 2, ... within the same entry
  accountCode TEXT NOT NULL,       -- e.g. '1100' (AR), '4000' (Sales), '2400' (GST output)
...
CREATE INDEX IF NOT EXISTS idx_lje_source     ON ledger_journal_entries(sourceType, sourceId);
CREATE INDEX IF NOT EXISTS idx_lje_account    ON ledger_journal_entries(accountCode, postedAt DESC);
CREATE INDEX IF NOT EXISTS idx_lje_org        ON ledger_journal_entries(orgId, postedAt DESC);
CREATE INDEX IF NOT EXISTS idx_lje_unreversed ON ledger_journal_entries(reversedById) WHERE reversedById IS NULL;
