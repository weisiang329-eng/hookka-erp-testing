...
**Root cause.** `src/pages/reports.tsx`:

```ts
const seed = (s: string) => { let h = 0; for (…) h = (Math.imul(31, h) + s.charCodeAt(i)) | 0; return Math.abs(h); };
const s = seed(w.id);
...
A hash of the worker's **primary key**. Deterministic, so each person carried the same invented performance on every visit — which is exactly what made it look like data rather than a placeholder — and it could only change if their id changed. The date-range picker above it was decorative: nothing on the tab ever read `from`/`to`. `presentRate = 94.5`, `avgHoursPerDay = 8.7`, `workingDays = 22` and the `"12.5"` OT figure were typed into the source, and two further rows ("Total Present Days" / "Total Absent Days") were arithmetic **on** the invented 94.5%.

**Fix — every figure now names its source, and the range picker works.**

| Column / card | Now sourced from |
|---|---|
| Efficiency % | `GET /api/department-performance?view=summary&from&to` — `productionMinutes ÷ workingMinutes`, denominator = clocked time from `working_hour_entries` |
| Clocked Hours (was "Hours Worked") | the same call's `workingMinutes` — the actual denominator, so the ratio can be checked by eye |
| Job Cards Completed (was "Items Completed") | distinct `jobCardId`s the worker was credited on in range |
| Days With Attendance / Punch Records / Workers Who Punched / Total Clocked Hours / Avg OT Hours per Worker | `GET /api/attendance?from&to` |
| Avg Hours / Recorded Day (was "Avg Hours/Day 8.7") | clocked minutes ÷ records that have a clock-out |
| **Attendance Rate** | **"—"**, with the reason printed under the table |

Prod for 2026-06-14 → 2026-08-13 returns **38 workers spanning 28% – 186% efficiency and 312 – 2,270 job cards**, range total **81%** — it varies with the factory, which the hash never did.

**Why Attendance Rate is a dash and not a number.** `attendance_records` gets a row only when somebody punches, so every row is a presence: **2,780 of 2,780 records across all of 2026 carry status `PRESENT`, and the table contains no `ABSENT` row at all.** `present ÷ records` is therefore 100% by construction. Absence is derived elsewhere — `labor-engine.ts` calls a day absent when it has no logged hours — and is never materialised here. The card is gone; the Overview row reads `"—"` and states this. Two workers in the live range (`Violet`, `MYINT TUN`) clocked no minutes at all: they render `"—"`, not `0%`.

**`view=summary` is a projection, never a second computation.** The full `/api/department-performance` payload carries `daily[].jobs` and `daily[].workers[].jobs`, one entry per card per day — **9.5 MB for a 61-day range**, which the /employees drilldown needs and a report does not. `projectPerformanceSummary` drops the two drilldown arrays and folds the per-day worker rows the handler already produced into range totals: **9,573 KB → 10 KB, a 957× reduction**, with byte-identical figures (verified by running the projection over the live payload in the browser). The efficiency formula is not re-expressed — the range value is the same `productionMinutes ÷ workingMinutes`, so a per-worker row cannot disagree with the daily rows it was folded from. It is applied at BOTH return points; applying it only to the fresh-compute path would have let a snapshot hit silently return 9.5 MB. It is deliberately NOT in the snapshot `cacheKey` — one cached full payload serves both views.

**Verified.** Live figures above read off prod 2026-08-13. `tests/no-fabricated-worker-metrics.test.mjs` (4 tests) asserts `seed()` and its three formulas are gone, that none of `94.5` / `8.7` / `"12.5"` / `workingDays = 22` is rendered, that efficiency and clocked hours come from `/api/department-performance?view=summary` windowed by the tab's range and degrade to `"—"` (never `0%`) for a worker with no clocked time, that attendance is fetched with `?from=&to=` and never bare, that the attendance rate is refused **with its reason on screen**, that both renamed headers appear in the table AND the CSV, and that `projectPerformanceSummary` touches no table and re-uses the daily expression at both exits. A fifth case runs `projectPerformanceSummary` itself and asserts the fold reconciles with the daily rows it came from (a card spanning two days counts once; a worker credited on a card but never clocked in comes back with 0 and it is the PAGE that must render that as "—"). Full suite green; `npx tsc -p tsconfig.app.json --noEmit` clean; `npm run build` clean; eslint clean. **Not observed on prod** — the branch is not deployed, and the login gate means the UI could not be exercised locally either; every figure above was read from the live API, not from a rendered page.

**Not fixed here.** `/api/reports/efficiency.json` (`src/api/lib/efficiency-report.ts`) is a second, trustworthy clocked-time surface that this page still does not use; it was not needed once `department-performance` covered the columns. And the Employee tab's Attendance Overview counts punch records, not scheduled days — a real *expected-days* denominator would need a shift calendar this system does not have.

---

## BUG-2026-08-13-005 — Reports › Production said "No data available" over a request that had been killed at 30,012 ms `ui-frontend` `production-orders` `performance` `data-integrity` 🟢

**Symptom.** Prod, 2026-08-13: `/reports` → **Production** → 2026-06-14 → 2026-08-13 → **Generate** rendered an **EMPTY Department Efficiency table captioned "No data available"**. The caption was false. `GET /api/production-orders/tracker-summary` reports **370 job cards completed on 2026-08-11 and 367 on 2026-08-12**; the correct report for that window is **1,113 production orders, 820 completed, 3 in progress, avg completion 10.5 days, 8 department rows (1,977 completed cards in Fabric Sewing alone) and 38 overdue orders**.

**Root cause — two independent defects, and the second is the dangerous one.**

1. The tab fetched **bare `/api/production-orders`** — every PO in the org **with every job card**, ~2,539 orders / 36,796 cards — purely so a `forEach` could add them up in the browser. Measured **30,012 ms** on prod, past `API_TIMEOUT_MS` (30 s) in `src/lib/api-client.ts`, so the request was aborted and never resolved. Same family as BUG-2026-08-13-001 / -002 / -003.
2. **A failed fetch and an empty result were written into state identically.** `cachedFetchJson` returns `null` for a timeout, an abort, a 500 and a degraded `_stub` body alike, so `json?.data || []` stored `[]`; the `catch` block stored `[]` too. `ReportTable` then printed **"No data available"** — a factual claim about the factory — over a dead request. All **five** tabs shared this shape, so this was a page-wide pattern, not one instance.

**Fix — 1: aggregate in SQL.** New **`GET /api/production-orders/report-summary?from=&to=`** (`src/api/routes/production-orders.ts`, registered before `/:id`) returns the whole tab: status counts in the window, the completed orders' two dates for the average, the per-department paired subtotals, and the overdue list with `daysOverdue` already computed. Four small aggregate queries, one small payload; **`from`/`to` are required and 400 if malformed**, so the endpoint cannot degenerate into the unbounded scan it replaces. No job card ever reaches the browser again.

**Fix — 2: a report that cannot load says so.** New `cachedFetchJsonResult` (`src/lib/cached-fetch.ts`) returns `{ ok, data, error }` — additive, `cachedFetchJson` is untouched for its ~80 other callers. All five tabs now branch on `ok`, keep `data` at `null` on failure, and render a new `ReportError` card: the message, a **Retry**, and the sentence *"this is not a statement that there was no activity in this range."* The 30 s abort gets its own wording ("This report took too long to load. Narrow the date range and try again.") because `humanizeError` would otherwise hand the operator the AbortError's own word, "Aborted". Tabs with two sources (Sales, Financial) fail as a whole — a P&L with a loaded revenue side and a failed payable side is wrong, not partial.

**Also found, and it inverts a premise. `actualMinutes` is populated but it is a COPY of the estimate.** BUG-2026-08-13-004 correctly established that **4,340 of 36,796 job cards carry a non-null `actualMinutes`, 4,289 non-zero** — and that blanking the metric on an assumption of absence would have hidden them. Counting the column one step further on prod: **on all 4,289 of those cards `actualMinutes` is byte-identical to that card's own `estMinutes` — 4,289 of 4,289, zero exceptions**, and they sit entirely on orders started in 2026-04 (4,217) and 2026-05 (72). So a `standardMinutes ÷ recordedMinutes` ratio over them is **100.0% by construction** — the same fabricated number -004 removed, arriving through a different door: April 2026 rendered "100.0%" in all eight departments. The endpoint therefore also returns `measuredDistinctCards` (measured cards whose recording actually **differs** from the estimate; currently 0 org-wide), and the page publishes a percentage only when that is > 0. Otherwise the cell reads `"—"` and the caption says why — that the recording is a copy of the estimate, not a measurement. **The Measured Cards column still shows the real recorded count**, so nothing is hidden.
...
**It was never the network.** All 22 API calls on that page total **0.17 MB**; the slowest real one, `department-performance`, answers in **902 ms**. Memory is a modest 38 MB. The entire cost is DOM layout and paint.

**Root cause.** The Working Hours tab — the tab `/employees` opens on — renders **every working-hour entry in the selected range**, and the owner's ruling is that every tab opens on the CURRENT CALENDAR MONTH (`readPersistedDateRangeThisMonth`). A month is ~700 entries, and each row is not a row of text: it carries a department `<select>` (15 options), a category `<select>`, a worker `<select>` listing every ACTIVE employee, three inputs and three buttons. Prod measured **1 table, 695 `<tr>`, 46,137 nodes** — roughly **66 nodes per row**, none of it windowed. Every other page in this app is 900-1,900 nodes. That table alone was **44,328 px** tall, so each scroll frame had to re-lay-out all of it.

**Fix — window the grid, snapped to whole groups.** `WorkingHoursTab` (`src/pages/employees.tsx`) now scrolls **inside its own `max-h-[70vh]` box** and renders only the visible window, through a new primitive:

- `src/components/ui/virtual-groups.tsx` — `useVirtualGroups`, the sibling of `useVirtualRows`.
- `src/lib/virtual-group-window.ts` — the pure arithmetic, unit-tested without a DOM.

It could **not** reuse the existing flat `useVirtualRows`, for two reasons that are properties of this grid, not preferences: (1) a worker-day's Date / Employee / Punch cells are one `<td rowSpan={n}>`, so half a group is not renderable and the window must snap to group boundaries; (2) the row heights are not uniform — measured on the rendered grid, a **one-row group is 71 px** (its Employee cell stacks the worker `<select>` above the day-total chip) while a grouped row is **50 px**. A single constant drifts the scrollbar by thousands of pixels over 600 rows. The two heights are now the named constants `WH_ROW_SOLO_PX` / `WH_ROW_SEG_PX` with a "re-measure if the markup changes" note on them.

**Every existing behaviour is preserved, by construction.** Sorting, the three column filters, per-row and bulk save, duplicate/remove, the punch cells and the day-total chips all address `originalIdx` into the flat `rows` array, and Save All / Print Report read `rows` / `filteredRows` — never the DOM. So a row that scrolls out of the window is unmounted, not forgotten.

**Verified locally** (this branch is NOT deployed; the numbers below are measured on `npm run dev` against a stub API serving a prod-sized 666-entry / 600-group month, at 1280×900):

| | before | after |
|---|---|---|
| DOM nodes | 49,126 | **1,496** (−97%) |
| `<tr>` rendered | 666 | 8 + 1 spacer |
| rows in the model | 666 | **666** (unchanged) |
| page height | 45,322 px | 1,616 px (grid box scrollHeight 44,551 — the scrollbar still tells the truth) |
| initial-load long tasks | 10,914 ms total / 10,414 ms blocking / worst 4,617 ms | **1,351 / 901 / 306 ms** |
| style-recalc + layout of the grid | **675-958 ms** | **10.6-11.5 ms** (−98%) |
| one filter keystroke (sync) | 976 ms | 587 ms |

That style-recalc + layout figure is the freeze: it is the work every scroll frame had to redo.

**Correctness checked, not assumed.** Scrolling the windowed grid top to bottom surfaced **666 unique rows in exactly the expected order, zero mismatches, 100 % coverage** against the API payload's own grouping. Sort cycles asc → desc → cleared; the employee filter narrows to 24 groups (below the 40-group threshold, so windowing switches off and the result is byte-identical to the pre-fix render: 26 rows / 2,792 nodes); a dept filter leaves 44 groups and stays windowed. An inline edit made in the window survived scrolling 25,000 px away and back, on the right row, with `Save All (1)` counting exactly one dirty row. Add Row still appends a blank draft at the bottom; Duplicate still turns a group into a `rowSpan=2` pair, and the two rows measure 50 px each — exactly what the height model predicts. Checked at 1280×900 and 1024×768; below 768 the app redirects to the `/m` shell, so this grid is desktop-only.

**Could NOT be measured: scroll long-tasks.** The verification browser's pane is not displayed, so the page never composites frames and `requestAnimationFrame` never fires — a scripted scroll cannot be timed there. The style-recalc + layout number above is the honest proxy; the real before/after scroll blocking has to be taken on prod after deploy.

**Found, NOT fixed — every page chunk statically imports the 1,036 KB `pdf` chunk.** `/employees` was seen pulling `pdf-*.js` on mount. It is not employees.tsx's doing: no module in its static graph imports jspdf / pdfjs-dist / html2canvas (traced mechanically), and its payslip generator is correctly behind `await import()`. The build tells the real story — `dist/assets/employees-*.js` contains `import{c as r}from"./pdf-D5mT946N.js"`, and so do **~30 other chunks** including `dashboard-b`, `customers`, `delivery`, `accounting` and `consignment`. That symbol is **`__vitePreload`**, the dynamic-import preload helper, which rolldown parked inside the `pdf` manual chunk — so *any* chunk containing *any* `await import(...)` drags a megabyte of PDF vendor code in as a static dependency. `vite.config.ts`'s `modulePreload.resolveDependencies` filter does not help: this is a hard import edge, not a preload hint. The fix belongs in `manualChunks` and would change how every page loads, so it needs its own PR and its own bundle-size baseline.

**Gates.** `npx tsc -p tsconfig.app.json --noEmit` clean · full suite **3,713 pass / 0 fail** (incl. 9 new in `tests/virtual-group-window.test.mjs`) · eslint 0 errors on the changed files (one `react-hooks/incompatible-library` warning on `useVirtualizer`, identical to the one the existing `virtual-rows.tsx` carries) · `scripts/check-bundle-size.mjs` — the `employees` chunk moved **276,884 → 278,503 bytes (+0.58 %)**, no new regression; the gate's 4 failing stems (`verified-save`, `BUG-HISTORY`, `reports`, `finance-dashboard`) are pre-existing drift from other branches and contain none of the files this change touches.

---

## BUG-2026-08-13-008 — Class sweep: every remaining quadratic list join, measured. Four fixed, twelve proven cheap, one doc correction `performance` `data-integrity` 🟢
...
**Root cause.** The tile's gate was `pendingL = poL || doL || poValL || soItemsL`, and `poL` belonged to a **bare, unscoped** `useCachedJson("/api/production-orders?fields=minimal&include=jobCards")` — every one of ~2,539 POs **with all of their job cards**, megabytes on the wire, fetched by the browser purely so a `useMemo` could run `poReadyForDelivery` over it and sum the ringgit. That request can exceed the global 30s abort in `src/lib/api-client.ts` (`API_TIMEOUT_MS = 30_000`); on abort the hook's loading flag **never clears**, so the gate stays true forever. Three more whole-org fetches (`/po-values`, `/linked-po-ids`, `/api/sales-orders?fields=price-index`) hung off the same gate. `doL` — the capped 200-row DO page — was still in the gate too, despite the comment above it stating the payload "is no longer READ for any KPI": it was pure dead weight holding the tile hostage.

**Fix.** Move the sum to the server: **`GET /api/delivery-orders/pending-value`** (`src/api/routes/delivery-orders.ts`) returns `{pendingDeliveryValueSen, readyCount}` and the tile reads that one number. The readiness rule was **NOT** re-expressed in SQL — the endpoint calls the existing `loadDeliveryReadyPlanning()` (the `/ready-planning` body, extracted so both share one assembly *and one snapshot*) and sums `valueSen` over the rows the shared `buildReadyPlanning()` → `poReadyForDelivery()` already produced. That predicate carries too much hard-won behaviour to retype: sofa upholstery cards, HB-only bedframes' stranded DIVAN cards, ACCESSORY POs that never get an UPHOLSTERY card (BUG-2026-06-20-001), `ON_HOLD`, `CANCELLED`, consignment-sourced POs, and POs already on a live DO. Each row's `valueSen` is the identical `poValMap.get(po.id) ?? soLinePrice × qty` expression the browser used, so the figure is unchanged by construction. Four heavy fetches deleted from `src/pages/dashboard-b/index.tsx`; the dispatch-chain fold (`/delivery-orders/stats`) is untouched.

**Money discipline.** Integer sen only — `valueSen` is an int and a sum of ints is exact; no float is introduced. `tests/pending-delivery-value.test.mjs` (12 tests) runs the **old client-side expression and the new server summation over the same dataset and asserts they agree to the sen**, covering: a PO already on a DO excluded (and re-included when unlinked), `ON_HOLD` + `CANCELLED` excluded, consignment excluded, a COMPLETED ACCESSORY with no UPHOLSTERY card INCLUDED, po-value-vs-SO-price fallback, and that an **explicit zero po-value does not fall through to the SO price** (`??`, not `||`). Two existing source guards were re-pointed rather than deleted — `tests/pending-stage-complete-set.test.mjs` now pins the BUG-2026-06-27 "complete linked-PO set" invariant at its new server-side home, and `tests/customer-scope-sql.test.mjs` follows the scope clauses into the shared function.
...
**Symptom.** A staff member opened a Service Case, clicked **Spawn Service Order**, chose Resolution Mode **Reproduce**, added one Affected Item with **CODE blank** (the column was labelled `optional`), PRODUCT NAME `1041 (Q)`, QTY 1, ISSUE "HB & Divan Broken" — and got a red toast reading **"Something went wrong. Please try again."** Reproduced against prod: `POST /api/service-orders` → **400**, body `insert or update on table "production_orders" violates foreign key constraint "production_orders_product_id_fkey"`. The product exists: the catalogue code is **`1041-(Q)`** (`prod-53`, VICTORIA BEDFRAME (5FT)). The operator typed a **space where the code has a hyphen**.

**Root cause — four faults stacked.**

1. **Reproduce needs a real product; the screen said it didn't.** REPRODUCE inserts one `production_orders` row per line and `production_orders.product_id` is a FK to `products` (confirmed on prod: nullable TEXT, `production_orders_product_id_fkey`). The route bound `productId ?? ""`. **An empty string is not NULL**, so a nullable FK column still runs its check against `""` and Postgres rejects the whole transaction. The dialog, meanwhile, hard-coded `productId: null` for every free-text line and told the operator the code was optional.
2. **The operator was shown a constraint name — and then something worse.** The route's catch-all returned `err.message` raw. `toast.error` runs `humanizeError`, which correctly classifies `violates … constraint` as technical and replaces it with the generic line — so the toast was useless *because* the message was unusable, and retrying could only fail identically.
3. **Nothing helped them find `1041-(Q)`.** It was a free-text box next to another free-text box, on a screen that had a perfectly good searchable picker available.
4. **STOCK_SWAP could never have worked either.** Every "FG Batch" dropdown in the module is populated from `/api/inventory`, whose `finishedProducts` is the **products** table (and whose `stockQty` is hardcoded 0, so "(0 on hand)" was always a lie). The id posted as `resolutionFgBatchId` was therefore always a `prod-*` id, which the route looked up in `fg_batches` and 404'd. Prod bears this out: **all 7 service orders ever created are REPAIR** — the only mode that writes nothing.

**Not a bug (checked first, because it would have outranked the rest).** The insert sequence **is** atomic: `SupabaseAdapter.batch` wraps the statements in `sql.begin(...)`, so the FK rejection rolled the SVC row, its lines, the PO and the `service_cases → IN_PROGRESS` flip back together. Prod confirms zero orphans: 7 service orders, 0 with zero lines, 0 orphan lines, 0 orphan `production_orders`, 0 REPAIR POs with a blank/null product, 0 cases sitting IN_PROGRESS without an order. `nextServiceOrderNo` is a `COUNT(*)`, so no number was burned either.

**Fix.** The requirement is now **derived from what each mode writes**, stated once, and read by both sides — `src/lib/service-order-modes.ts` (new, pure): REPRODUCE writes a product-bound `production_orders` row → catalogue product required; STOCK_SWAP consumes an `fg_batch`, which belongs to exactly one product → required; REPAIR writes neither → free text allowed, deliberately, because a case can be about something we never sold. `validateSpawnLines` is called by the dialog (`src/pages/service-cases/detail.tsx:3140` region) *and* by `POST /api/service-orders` (`src/api/routes/service-orders.ts:600`) and `PUT /:id/mode` (`:1279`), so the screen can no longer promise what the route refuses.

`normaliseProductKey` folds a typed reference to `[A-Z0-9]`, so `1041 (Q)`, `1041-(Q)` and `1041(q)` all resolve to `prod-53`; verified against prod that folding all **375** product codes produces **zero** collisions, so a fold-match is safe to accept outright. A fold that hits several rows is reported as ambiguous, never guessed. The Affected Items row now **picks** via the existing `SearchableSelect` (`:3260` region) with a "Not in the catalogue" escape hatch that is explicit about being Repair-only, and the refusal is shown inline per row before the button is ever pressed.

The route refuses **before composing a single statement**, so a refusal names the row the operator can see and ends "Nothing was created". `production_orders.product_id` is never bound `""` again (`:640`, `:1300`). Seven catch-alls now go through `operatorSafeError` (`src/lib/humanize-error.ts:144`), which keeps a hand-written sentence and swaps anything `looksTechnical` for a plain per-route fallback. STOCK_SWAP derives its batch FIFO from the resolved product (`resolveSwapBatch`, `:63`) and tolerates a product-id hint, which repairs every existing picker at once; the dead FG column came out of the spawn dialog and the mode-change dropdown was relabelled (`src/pages/service-orders/detail.tsx:906`).

**Deliberately left.** `CreateServiceOrderModal` in `src/pages/service-orders/index.tsx` (~900 lines) POSTs without `caseId`, which the route has required since 0074 — but `setCreateOpen(true)` is never called and nothing imports it, so it is unreachable dead code, recorded in `BUG-CLASSES.md` C13 #4 rather than half-migrated here.

**Verified.** `tests/service-order-spawn-product.test.mjs` (25 tests) drives the real handlers against an in-memory book whose `production_orders` insert **re-implements the FK** — an unknown id, including `""`, throws the exact string prod threw — and whose `batch()` is atomic like `SupabaseAdapter.batch`. It pins the reported click now succeeding with `product_id = prod-53`; an unmatchable line refused with a message that starts `Line 1 (…)`, says what to do, and passes `looksTechnical === false`; **nothing written** on refusal (no order, no line, no PO, case still OPEN, `batch()` never even called); REPAIR spawning fine with no code at all and opening no PO; STOCK_SWAP drawing the oldest batch and refusing in plain English when there is none. Full suite 3,632 pass / 0 fail; `npx tsc -p tsconfig.app.json --noEmit` clean. Prod figures above read live, read-only. **Not yet exercised in a running app or on prod** — merged to `main`, not pushed.
...
**Root cause.** `src/pages/sales/edit.tsx` hand-rolled the unit price as `base + divan + leg + special` (`:775` before the fix) and never mentioned `totalHeightPriceSen` anywhere — not in its `LineItem` type, not in the load mapping, not in the PUT payload. Since migration 0209 the total-height surcharge is the FIFTH stored component, and `PUT /api/sales-orders/:id` resolves it with `resolveTotalHeightPriceSen`, whose trust model DERIVES the value from gap+divan+leg whenever the client OMITS the field (BUG-CLASSES C1). This page omitted it on every save, so the server derived it and folded it into `unitPriceSen` — a number the screen had never shown. The inline build-up (`:1556`) re-summed the same four components with no reconciliation guard, so it printed a decomposition that could not equal the charge.

**Fix.** Both order EDIT screens now route through the shared module instead of carrying their own copy of the sum. `src/lib/pricing.ts` gains `orderLinePriceBuildUp` + `formatOrderLineUnit` (the order-side translation of `invoicePriceBuildUp`, rules 3 and 4 of `src/lib/invoice-line-price.ts`: a build-up is shown only when it sums to the charge and only when there IS a surcharge; otherwise the single price renders). `sales/edit.tsx` carries `totalHeightPriceSen` in state, seeds it from the line's own stored column (never re-derived on load, so Save-without-typing is a no-op), re-derives it with the server's own `deriveTotalHeightSurchargeSen` when gap/divan/leg change, and posts it — so the PUT trusts the displayed number rather than inventing its own. `consignment/edit.tsx` routes through the same helpers and passes `totalHeightPriceSen: 0` **explicitly**, because the CO write path (`consignment-orders.ts` POST + PUT) charges four components and never fills `consignment_order_items.total_height_price_sen`; the CO screen must show what the CO save charges. **No stored order was touched** — the change is confined to two screen components and one pure helper module; nothing re-derives, rewrites or migrates an existing row.

**Open (not fixed here, needs an owner pricing decision).** `src/pages/consignment/create.tsx` computes a total-height surcharge, displays it in its unit price and POSTs `totalHeightPriceSen`, and `consignment-orders.ts` drops it on the floor — so a NEW consignment order with a priced total height is created worth less than the create screen showed. That is a CO write-path gap (should consignment orders carry a T.Height charge at all?), not a rendering bug, and repricing consignment orders is a money decision.

**Verified.** `tests/order-edit-unit-price.test.mjs` asserts behaviour: displayed unit === saved unit on a line with a T.Height component (and the old four-term sum is asserted NOT to equal it); a payload omitting the field is shown to be repriced by the server; Save-with-nothing-typed charges what was already charged across stored surcharges including hand-set ones; a height change moves screen and save together; service orders stay free; a non-reconciling build-up renders as the single price with no decomposition. Plus source guards that neither screen re-grows its own sum or its own build-up renderer. `npx tsc -p tsconfig.app.json --noEmit` clean; suite green except one unrelated pre-existing failure in another agent's in-flight `delivery-orders/_helpers.ts`. Live prod verify pending deploy.

---

## BUG-2026-08-07-006 — An invoice line printed a price build-up that did not add up, and pressing Edit → Save without typing anything repriced it `invoices` `money` `ui-frontend` 🟢

**Symptom.** Two faces of one disease, both reported by the owner ("为什么 backend 和 frontend 不统一的呢?"). (A) The invoice detail screen displayed "Base … = RM 305" on a line whose Base/Divan/Leg/T.Height/Special components summed to RM 307.98 — a decomposition contradicting the charge. The PDF of the same invoice refused to print it. (B) Clicking **Edit Prices** on that line pre-filled the five fields from those same components, so Save **with nothing typed** wrote unit = RM 307.98 over the charged RM 305.00. A silent reprice from a no-op.

**Root cause.** A line's price build-up was derived in four places, each with its own copy of the rule: the backend resolver (`invoice-print-extras.ts`), the invoice detail read view, its edit seed, and the PDF builder. Only the PDF carried the reconciliation rule (`build-unified-doc-data.ts:60`, `base+divan+leg+totalHeight+special !== priceSen` → don't render). The screen had no guard, so it showed what the PDF refused; the editor seeded from the same unguarded build-up, so it re-summed to a different charge. The resolver made it worse by reporting the SALES ORDER's `unitSen` as the line's unit — a number that need not be the price the invoice charges. `f0f002b4`'s scoping fix (`poLink → poToSo → tightBySo`) stops the WRONG sales-order row being picked; it cannot help a line with no PO link (~7.4%), where a wrong row is picked anyway.

**Fix.** The rule is now WRITTEN DOWN and held in one module: `src/lib/invoice-line-price.ts` (importable by both sides; `src/api/lib/` is backend-only). The charge is `invoice_items.unitPriceSen` and is authoritative; the build-up only EXPLAINS it; an explanation that does not sum to the charge is never displayed (`invoicePriceBuildUp`); editing moves explanation and charge together, so the editor's seed always reconciles (`invoicePriceEditSeed` — a resolved build-up verbatim when it reconciles, otherwise the charge as Base with the four surcharges at 0, which by the no-surcharge rule is never re-rendered as a fake decomposition, and carries a "No itemised build-up on file" note). Every caller goes through it: `invoicePriceBreakdown` (`build-unified-doc-data.ts`, the Invoice/SO/CO PDF seam), the invoice detail read view and editor (`invoices/detail.tsx`), `priceLines` (`generate-invoice-pdf.ts`), the resolver (`invoice-print-extras.ts` — which now returns the CHARGE as `unitSen` plus a `buildUpReconciles` verdict), the `priceEdits` write (`api/routes/invoices.ts`), and the order-side alias `calculateUnitPrice` (`lib/pricing.ts`). No stored price was touched — read/seed only.

**Verified.** `tests/invoice-price-buildup-rule.test.mjs` asserts behaviour, not shape: a non-reconciling build-up is not displayed on screen and not printed; a no-op Save charges exactly what was already charged across a table of lines and charges (including the RM 305 → RM 307.98 case, odd cents and zero); typing still moves the charge; and the real backend resolver, the screen and the PDF reach the same verdict and quote the same charge on the same line. `npx tsc -p tsconfig.app.json --noEmit` clean, full suite green (3220). Live prod verify pending deploy.

---

## BUG-2026-08-07-005 — Bill ONE line of a ten-line goods receipt and the other nine could never be corrected again `purchasing` `data-integrity` 🟢

**Symptom.** A GRN with ten lines, one of which a purchase invoice had billed. Every "Edit Quantities" attempt — on any line, in any direction — 409'd with "This GRN has been billed on a purchase invoice. Delete the linked PI first." The nine untouched lines were uncorrectable while that invoice lived, and the detail page hid the edit button outright.

**Root cause.** `isGrnLockedByDownstreamPi` (`purchase-edit-rules.ts:76`) returned true when ANY line had `invoiced_qty > 0`, and `grn.ts:1800` 409'd the whole document on it. The owner ruling it implemented (2026-06-29) was right about the LINE — an invoiced quantity must not be edited out from under a live invoice, and even an up-edit leaves the PI on a stale qty/price — but it was applied at document scope. Owner rule: "这种转换不是整张单锁死的" — a conversion locks the line it consumed, never the whole document.

**Fix.** The lock moved into `checkGrnLineQtyEdit` (`purchase-edit-rules.ts:161`), which already ran per line on both the FE and BE: a line with `invoiced_qty > 0` is frozen in both directions, every other line moves with the usual compensating stock movement, and delta 0 always passes so unchanged lines save around the frozen ones. `isGrnFullyLockedByDownstreamPi` (`:98`) keeps a document-level 409 (`grn.ts:1804`) only for the degenerate case where EVERY line is billed. `grn-detail.tsx` now marks each locked line with a Lock chip + invoiced qty and a "N of M lines locked" banner, and renders those inputs read-only while opening the rest; `saveQtyEdit` (`:378`) pins a locked line to its stored qty so a stale edit buffer can't smuggle a change past the identical backend rule.

**Verified.** `tests/purchase-edit-cascade.test.mjs` drives the real handler: a two-line GRN with the first line billed corrects the second 8→6 with stock following (balanceQty and the lot both −2) while the billed line's qty, stock and lot stand still, and that billed line still 409s on its own edit. Unit rules pinned in `tests/purchase-edit-rules.test.mjs`. `npx tsc -p tsconfig.app.json --noEmit` clean, full suite green (3163). Live prod verify pending deploy.

## BUG-2026-08-07-004 — A POSTED goods receipt was immortal: a receipt keyed against the WRONG purchase order consumed that PO line forever `purchasing` `inventory-cascade` `data-integrity` 🟢

**Symptom.** Receive goods against the wrong PO and there was no way back. Un-post 409'd ("Reverse it with a stock adjustment instead"), delete 409'd with the same line, and neither the GRN list nor the detail page exposed a delete or void at all — the DELETE route was unreachable from the UI. The wrong PO line's `receivedQty` stayed consumed and the order stayed RECEIVED. Worst of all, local goods-in-hand are BORN POSTED (`grn.ts` POST, arrival ARRIVED → POSTED), so the unreversible case was the COMMON case. The only escape was editing every line's quantity down to 0.

**Root cause.** GRN had no CANCELLED status — the model was DRAFT/CONFIRMED/POSTED only (`grn.ts:317`) — so the 2026-06-21 "option A" lock had nothing to transition to. It was correct that a bare status flip must not un-post (that frees the PO line while the stock stays, a double-count hole); it was wrong that no guarded path existed at all.

**Fix.** `POST /api/grn/:id/cancel` (`grn.ts:2230`), schema self-applied by `ensureGrnCancelSchema` (`grn.ts:118` — `cancelled_at`/`cancelled_reason` plus a status CHECK drop-and-re-add to admit CANCELLED, each statement individually try/caught rather than run through `runSelfApply`, because one legacy row holding an unlisted status would otherwise take every GRN write down; mig 0213 is the record copy). It reverses exactly what `postGRNToStock` wrote, driven off the LOTS (`rm_batches WHERE source='GRN' AND sourceRefId=?`) rather than the lines — a line whose material never resolved got no lot, and a later qty edit moved `originalQty` in step with `balanceQty`, so the lots are what actually landed. Per lot: `balanceQty += −originalQty` (the same signed statement the post and edit paths use) and DELETE the lot row. The PO is restored by `restorePOReceivedQtyForGRN`, the SAME helper delete and un-post already use — no third restore path.

**No cost_ledger leg, deliberately.** The FIFO/P&L engine sources GRN receipts from `grn_items JOIN grns … WHERE g.status IN ('CONFIRMED','POSTED')` (`accounting.ts` `loadMaterialCostData`) and reads cost_ledger only for RM_ISSUE/ADJUSTMENT. Flipping to CANCELLED removes the receipt from the replay by itself; appending an ADJUSTMENT OUT on top would reduce material cost TWICE. The original RM_RECEIPT leg is left untouched — the ledger is append-only.

**Two refusals, because a partial reversal is worse than none.** (1) A live purchase invoice still drawing on the lines (`invoiced_qty > 0` or a non-CANCELLED `purchase_invoices.grn_id`) — the error names the PI and says voiding it returns the invoiced quantity here first. (2) Any lot already CONSUMED (`remainingQty < originalQty`): the goods are issued into production or returned, so there is nothing safe to hand back — the error names the material and the quantity used, and points at a purchase return or stock adjustment. Nothing is written on either path.

**Terminal, and honest about it.** The PUT 409s on any edit or resurrection of a CANCELLED row (checked before the items branch, whose pre-commit path would otherwise happily rewrite a cancelled document's lines). DELETE still refuses a POSTED GRN but now points at Cancel Receipt, and a CANCELLED row IS deletable — it holds no stock and no PO quantity, and the delete-path restore correctly skips it. UI: "Cancel Receipt" on the detail header and in the list row menu, both confirming with the PO number and the exact quantities released.

**Verified.** `tests/purchase-edit-cascade.test.mjs` against the real handler: cancelling a POSTED GRN drops the PO line 10→5 and the order out of RECEIVED, takes on-hand 5→0, removes the lot, writes NO ADJUSTMENT leg, and then refuses both a re-cancel and a resurrect; cancel is refused (nothing moved) with a live PI naming PI-9, and refused naming FOAM-1 when 2 of 5 had been issued. `npx tsc -p tsconfig.app.json --noEmit` clean, full suite green (3163). Live prod verify pending deploy.

## BUG-2026-08-07-003 — A purchase order could be invoiced TWICE for the same goods: the GRN-sourced invoice never met the PO's ceiling `purchasing` `money` `data-integrity` 🟢

**Symptom.** Bill a PO for its full 100 units, let the goods arrive, post the GRN, then raise a second invoice off that GRN for the same 100 — both are accepted. 200 of payable against a 100 purchase order, no 409 anywhere. The three-way match is derived and advisory; nothing blocked it.

**Root cause.** `purchase-invoices.ts` POST guarded the two sources in an `if (body.grnId) { … } else if (body.purchaseOrderId …) { … }`. The GRN branch validated ONLY `grn_items.acceptedQty − invoiced_qty` — the receipt's own remaining — and the `else` meant the PO branch's ceiling never ran for it. Billing off a receipt does not create fresh entitlement: the goods were ordered once. The REVERSE order was already caught, which is why this survived: the PO ceiling counts already-invoiced as `COALESCE(pii.po_id, pi.purchaseOrderId)`, and a GRN-sourced line carries `po_id`, so a GRN invoice consumed the PO's remaining and the later PO-direct invoice hit the wall. The hole was one-directional. The PUT re-line path was the same hole with an extra step (raise for 1, edit to 100) — it checked only the GRN ceiling too.

**Fix.** ONE ceiling, three callers — `checkPoRemaining` (`purchase-invoices.ts:945`), which the PO branch now calls instead of its own inline copy (`:1359`), the GRN branch calls after the receipt ceiling (`:1332`), and the re-line path calls with the edited PI excluded (`:2027`). Two ceilings that can disagree is how this was born. Requested qty is aggregated PER MATERIAL CODE before measuring, so two lines of one material can't each pass the same remaining. The ceiling itself is `poInvoiceCeiling(ordered, received)` (`convert-chain.ts:104`) = the GREATER of the two: a receipt may legitimately run over the order within tolerance, and goods accepted into stock must stay invoiceable. A GRN line resolves its PO as `line.poId → body.purchaseOrderId → grn_items.po_id → grns.poId`, and the INSERT writes the SAME resolution (`:1522`) so the order a line is measured against is the order it is counted against next time. No new column — `purchase_order_items.receivedQty`, `purchase_invoice_items.po_id` and `grn_items.po_id` already existed.

**Not over-blocked.** A GRN whose lines carry no PO (direct receipt) resolves to nothing and still invoices. A legitimate split (60 off the PO, then 40 off the GRN) still passes — the invoice being created isn't in the database yet, so it can't count against itself, and the re-line path excludes its own current lines via `AND pii.pi_id != ?` or every edit, including one that REDUCES the quantity, would 409.

**Error wording** (names the material, the ask, and what's left — "over the limit" alone just gets retried): `Line "WOOD-2" (Pine plank): requested 100 exceeds PO PO-2 remaining 0 — the PO allows 100 and 100 is already invoiced. Bill at most 0 against this PO, or amend the PO.`

**Verified.** `npx tsc -p tsconfig.app.json --noEmit` clean and the full suite green (3133 tests) in an isolated worktree off HEAD — the shared checkout had an unrelated workstream mid-flight. `tests/purchasing-convert-flow.test.mjs` drives the real handlers against the stateful mock D1: the exact sequence (PO-direct 100 → GRN-sourced 100) is REJECTED, resolution through `grn_items.po_id` alone is rejected too, 60+40 splits pass on both paths, a PO-less GRN invoices, an accepted over-receipt of 110 invoices but 111 does not, and the re-line walk-around is blocked while the edit that fits is not. Every one of those was proved by putting the bug back — the guard was disabled and they failed. Live prod verify pending deploy.
...
**2026-07-24 follow-up (FE half of the same bug)**: the Opening Balance grid preview computed totals WITHOUT the new opb405/305 sums, so the page showed Difference 1,136.44 CR while the server post-check would enforce 15,208.24 CR — the owner would have balanced the grid on screen and still been rejected (or worse, mis-keyed). Preview totals now mirror /opening-balance/post exactly and the derived 405/305 rows render in the grid like the 400 auto-row (7e3ee6dd, verified live: page maths replicated to the sen against the API payload — 242,204.56 DR / 257,412.80 CR). Grid seeding + keyable-account list both exclude SDC/SCC controls (verified 405-0000=SCC, 305-0000=SDC in prod COA), so a posted 405 leg can never double-count as a user row after re-post. Rule reaffirmed: any preview that mimics a server-side posting rule must read the SAME derived sums endpoint, never re-implement a subset.
...
**Follow-up (same session) — stray `{MODULE}` job card blocked PO completion:** after the relabel the PO `pord-co-72dcea0a-01` still read **PENDING (92%)** because ONE job card was stuck `WAITING` — a leftover from the old broken BOM whose `wip_key` was `5543-1C(LHF)::0::SOFA_BASE::Base {MODULE}` (the `{MODULE}` template placeholder never expanded). `recomputePoStatusAndProgress` requires every gated card COMPLETED/TRANSFERRED, so that one junk card held the PO open even though the physical sofa was done (other 12 cards COMPLETED, incl. all 3 final-assembly cards). Second audit-first one-shot `POST /api/import/backfill-complete-stray-jc-co2606002` (guarded: only acts if the target is WAITING **and** its wip_key contains `{MODULE}`) stamped that single card COMPLETED with the latest sibling date (2026-07-01) and recomputed. **Verified:** PO → **COMPLETED, progress 100%, 13/13 cards COMPLETED, 0 WAITING**. No inventory created (the FG unit already existed); the 12 completed cards untouched; NOT a re-explode/rebuild.
...
**Symptom (owner, live prod):** owner recorded a real supplier payment (HPV-2607-002, ADD WOORD TRADING SDN. BHD., RM 11,476.00 against PI-2606-037) via the normal Supplier Payment "Record Payment" form. It posted correctly and was visible in BOTH the Supplier Statement and the Creditor Ledger (balance reduced 41,850.00 → 30,374.00) — but the Supplier Payment page itself showed **"No supplier payments yet"**, "Total Paid RM 0.00", "Payments 0", even after a hard refresh. Diagnostic SQL on prod confirmed the row genuinely existed (`sp-ff52379d`, `org_id='hookka'`, `method='BANK_TRANSFER'`) with `purchase_invoice_id = NULL` — meaning the payment had gone through the **advance** path (unattached to any PI) rather than a direct PI allocation, despite the amount exactly matching PI-2606-037.

**Root cause (two independent bugs, same class):**
1. **GET /api/supplier-payments always returned an empty list.** The row-mapping loop read `row.payment_no` (snake_case). The Postgres adapter (`src/api/lib/db-pg.ts`, `transform: { column: { from: columnFrom } }`) camelCases EVERY result column regardless of the SQL's own (snake_case) alias text, so the field arrives as `paymentNo`. `row.payment_no` was therefore always `undefined` → `if (!payNo) continue` silently dropped every row — the list and all summary cards were always empty, for ANY supplier payment, on Postgres.
2. **PI-targeted allocations were broken** — likely the actual reason the owner ended up using the Advance field for this payment instead of paying PI-2606-037 directly. The per-allocation code (`POST /`, the restate/edit flow, and the same-session Knock-off feature) computed `outstandingBookedSen` / `outstandingSen` from `pi.amount_sen` / `pi.paid_amount_sen` (same camelCase-adapter issue) — always `undefined` → always netted to **0**. `computeAlloc` then rejects any partial `payMyrSen > 0` outstanding-0, or (for a `full` payment) books `bookedSen = 0` and the caller's `if (r.bookedSen <= 0) continue` silently skips the line — a specific-PI payment attempt would fail or silently do nothing, while the Advance field (which never reads `purchase_invoices` fields) worked fine. Separately, void/restate rolled back `Number(row.booked_sen)` the same broken way, so voiding or editing a supplier payment never actually undid the PI's `paid_amount_sen` (would double-count on restate, never decrement on void).
...
## BUG-2026-07-02-002 — Every Purchase Order showed a blank "Supplier SKU"; recovered on read by supplier + code + price `purchasing` `data-migration`
...
**Root cause:** the create-time snapshot of `supplierSKU` was never populated for ANY PO — the value is genuinely absent, not merely mis-read. (First hypothesis was a camelCase column-fold read miss and I shipped a dual-key read `r.supplierSKU ?? r.suppliersku`; it changed nothing on prod — 0/161 still blank, incl. a same-day PO — disproving "stored but unread".) The Supplier SKU is a property of the (supplier, material) binding, but that is NOT 1:1: one internal code can carry several supplier SKUs under one supplier — `PC151-01` → "COMFY IVORY SAND" @ RM7.50, "VANILLA" @ RM12, "STONE" @ RM12 — so it MUST be captured on the PO line at create time, and wasn't. Existing lines can't be blindly filled from (supplier, code) — that would show the wrong colour (a naive derive-from-binding pass was written then reverted for exactly this reason).

**Fix (recover by supplier + code + PRICE, unambiguous only):** the line's price disambiguates the variant. `loadSupplierSkuBindings` groups `supplier_materials` by `supplierId::upper(materialCode)` carrying each SKU's `priceSen`; `fillBlankSupplierSku` fills a BLANK line only when the match is UNAMBIGUOUS — exactly one binding for the code, OR exactly one whose price equals the line's `unitPriceSen`. Same-price variants (VANILLA/STONE both RM12) stay blank rather than risk a wrong SKU. A non-blank stored value always wins. Wired into `GET /` (one query per page) and `fetchPOWithItems` (detail + PDF). **Follow-up still open:** fix the create-time WRITE so the chosen SKU is snapshotted going forward (removes reliance on price recovery) — blocked on confirming the API layer actually redeploys (two earlier backend pushes appeared not to take effect on prod; this deploy doubles as that sanity check).

**Verified:** `tsc` clean. LIVE-VERIFY after deploy: `PO-2607-008` (line @ RM7.50) shows "MED- COMFY IVORY SAND"; RM12 same-price variants correctly stay blank.
...
🟢 **Fixed (prod, verified)** · `production-orders` · `infrastructure` · found during the 2026-06-24 "why is 559 in production" audit

**Root cause (confirmed by a 4-agent investigation):** the archive tables were created at migration 0038 via `CREATE TABLE … AS SELECT * … WHERE FALSE`, freezing their column list. Columns later added to the HOT tables reached prod only via runtime self-apply ALTER (migration files are inert on prod) and those ALTERs never touched the archive siblings — so the archives are missing `org_id`/`consignment_order_id`/`company_co_id`/`repairscope` (production_orders) and `branch_key`/`distributed_at`/`qr_token` (job_cards). `fetchFilteredPOs` + `fetchPaginatedPOs` built `(SELECT *, '' AS "archivedAt" FROM hot UNION ALL SELECT * FROM archive)`; mismatched column counts → Postgres rejects the UNION → **every archive-inclusive read 500s**. (Even at equal counts, `SELECT *` UNION ALL matches positionally — a latent corruption risk.)

**Fix (runtime self-apply):** new `src/api/lib/archive-union.ts` — a memoized helper that introspects `information_schema.columns` for both tables, `ALTER … ADD COLUMN IF NOT EXISTS` on the archive for every missing hot column (mirroring the hot column's real `data_type` so the positional UNION is type-compatible), then emits an EXPLICIT, identically-ordered, double-quoted column list used by both SELECT sides (quoting bypasses the camelCase→snake rewriter and locks positional alignment). Both UNION sites in production-orders.ts route through it; legacy `SELECT *` literal kept only as a fail-soft fallback. `includeArchive=false` byte-identical. Regression test tests/production-archive-union.test.mjs; build:strict clean (app + base).

**Second layer (data):** the schema fix makes the UNION RUN, but archive rows predating multi-tenancy have `org_id = NULL`, which the `WHERE orgId = ?` predicate filters out — so a one-time `UPDATE production_orders_archive SET org_id = 'hookka' WHERE org_id IS NULL` backfill is needed for archived POs (incl. "559") to actually surface. [verify-live + backfill done post-deploy — see session]
...
**Routing note:** shipped straight to `main` / prod (the owner wanted it live, consistent with the rest of this session's announcement + production work).
...
**Symptom / gap:** the worker phone popup ("Got it") recorded the acknowledgement ONLY in that device's localStorage (`hookka.worker.ackAnnouncements`). Three problems: (1) the office could not tell who had read an announcement — there was no per-worker read table at all; (2) the popup gate was purely device-local, so a worker on a fresh device, or after a cache clear, would have an EMPTY ack set and could be re-popped (or, worse, a shared/kiosk device's ack would suppress the popup for a different worker); (3) there was no way to chase the workers who hadn't read a notice.

**Build (per-worker server-recorded read-receipts):**
- **Schema (runtime self-apply — migration file is INERT on prod):** new `announcement_acks` table (`announcement_id`, `worker_id`, `acked_at`, PK `(announcement_id, worker_id)`) + index on `worker_id`, and a new `announcements.reminded_at` column. All created via the runtime `CREATE TABLE IF NOT EXISTS` / `CREATE INDEX` / `ALTER ADD COLUMN IF NOT EXISTS` inside `ensureAnnouncementsTable()` (`src/api/routes/announcements.ts` ~L70-130), awaited at the top of every ack handler — same load-bearing pattern as the `translations` column. snake_case columns, read dual-keyed (`r.ackedAt ?? r.acked_at`, `r.workerId ?? r.worker_id`). Inert record: `migrations-postgres/0188_announcement_acks.sql`.
- **Worker ack write:** new `POST /api/worker/announcements/:id/ack` (worker-token authed via the sub-app default-protect middleware, which now stashes the resolved `workerId` on the context). Idempotent upsert (`INSERT … ON CONFLICT (announcement_id, worker_id) DO NOTHING`) — a double-tap or retry is a no-op and never moves the original `acked_at`. Guards against orphan rows: an ack against a hidden/expired/unknown id is a no-op `{acked:false}`.
- **Server-driven popup gate:** worker `GET /api/worker/announcements` now also returns `ackedIds` — the active notices THIS worker has acked AND not been reminded about since. The phone seeds its popup-suppress set from `ackedIds` (reconciling to server truth for currently-active notices), falling back to the localStorage cache only when the field is absent (offline). Net effect: a fresh device re-pops every un-acked notice until the SERVER confirms the ack; a "Remind" drops the id from `ackedIds` so it re-pops.
- **Office read-receipt:** new `GET /api/announcements/:id/acks` (admin, `requirePermission read`) — LEFT-splits the ACTIVE worker roster (`workers WHERE status='ACTIVE'`, the canonical payroll.ts query; the `workers` table is bare-camelCase legacy) into `acked:[{id,name,empNo,ackedAt}]` (newest-first) and `pending:[{id,name,empNo}]`. Surfaced per-card on `src/pages/announcements.tsx` via a new collapsible `ReadReceiptPanel` ("Read X of N", two name columns, lazy-fetched on expand).
- **Remind (no push — re-pop is the only channel):** new `POST /api/announcements/:id/remind` (admin, `requirePermission update`) stamps `reminded_at`; the now-server-driven worker gate treats a notice reminded AFTER a worker's ack as un-acked again, so it re-pops on that worker's next app open. Returns the un-acked count for the office flash. The office FE confirms first (`useConfirm`) and flashes "re-pop for N". DELETE also cleans up the notice's ack rows.

**Worker FE (`src/pages/worker/index.tsx`):** `acknowledgeAnnouncements()` (~L510) now POSTs `/:id/ack` for each shown id (fire-and-forget, `workerFetch` is authed) IN ADDITION to the optimistic localStorage write (so the popup closes instantly, no flicker on a flaky network). `refreshAnnouncements()` (~L424) reconciles `ackAnn` from the server `ackedIds`. The re-readable banner and "seen" dot are untouched (separate keys).

**Regression test:** new `tests/announcement-acks.test.mjs` (registered in `package.json`) — ack idempotency (3 taps → 1 row), ack-guard (hidden/expired/unknown id writes no row), the worker GET `ackedIds` incl. the remind-since re-pop, the admin acked-vs-ACTIVE-roster split (inactive worker excluded entirely), the remind un-acked count + `reminded_at` stamp, and the auth gates (worker ack 401 without token; admin remind 403 for a read-only role / 401 with no role). Full suite: 1123 pass / 1 skip / 0 fail. build:strict clean (only the 2 known @zxing sandbox errors).

**Verify live:** worker A taps "Got it" → office Announcements page → expand "Read X of N" on that notice → worker A shows under Acknowledged with a timestamp; worker B (never opened the app, or a fresh device) shows under Not yet AND still gets the popup. Tap "Remind un-acknowledged (N)" → worker B's next app open re-pops the notice. **Lesson:** a "must-acknowledge" gate that lives only in device localStorage is invisible to the office and breaks on a fresh device — the acknowledgement must be a server row keyed by the token's worker id, with the popup gate reading back from the server.
...
**Symptom:** the QR printed on a Packing (FG) sticker encoded a deep link into `/worker/scan?op=FG-PACKING&po=…&pn=…` — a page that lives under `WorkerLayout`. `WorkerLayout` redirects ANY `/worker/*` route to `/worker/login` when `localStorage["hookka.worker.token"]` is missing, and every `/api/worker/*` call requires a valid `X-Worker-Token`. So a storekeeper without a Worker-Portal PIN who scanned a sticker landed on the PIN login screen and was blocked from the one thing they wanted to do — tell the system which warehouse rack the piece went onto.

**Root cause:** the sticker QR was a *gated* page link, not a public token. The existing public `/r/:rackId` rack-qr flow solves the OPPOSITE direction (scan the RACK, then scan items into it) and doesn't help someone who scanned an ITEM sticker and wants to assign a rack to THAT piece.

**Fix (mirror the public DO-QR model — the token IS the credential, scope = one forward write):**
- **Token model:** new `job_cards.qr_token` (snake_case; migration `0187` + runtime self-apply `ensureJobCardQrTokenColumn`, awaited before first write). 64 random hex chars (two `crypto.randomUUID()`), generated like `do-qr-token.ts`. Minted LAZILY only by the AUTHED endpoint `POST /api/production-orders/packing-rack-tokens` (`production-orders.ts` ~L5790, gated `requirePermission(c,"production-orders","read")` — same "show me the QR" gate as the DO qr-token endpoint), which resolves each `(poNo, pieceName)` to its ONE PACKING job_card (mirrors `resolvePackingCard`) and stamps the token. We do NOT put the bare (short, enumerable) job_card id in the public URL — a guessed id would let anyone rewrite the rack on any card. Lib: `src/api/lib/jobcard-qr-token.ts`.
- **Public route:** `src/api/routes/public-rack-write.ts`, mounted `app.route("/api/public/rack-write", publicRackWrite)` in `worker.ts` and added to `PUBLIC_PREFIXES`. `GET /:token` (strict `^[0-9a-f]{64}$` regex before any DB touch) returns a minimal NO-PRICE summary (poNo, SO no., WIP description, current rack) + the rack catalog; `POST /:token/rack` reads EXACTLY ONE body field (`rackingNumber`) and performs ONLY set/clear of the rack on the one token-resolved PACKING card — the `jobCardId` comes from the token's own row, never the request (tenant-safe).
- **No drift:** the validation + writes (PACKING-only, rack must exist in `rack_locations`, reject-don't-normalize, empty clears, mirror onto `production_orders`) are the SHARED helper `src/api/lib/packing-rack-write.ts` (`applyPackingRack`), now used by BOTH the worker `/packing-rack` path and the new public path.
- **Public page:** `src/pages/sticker-rack.tsx` at `/p/:token` (`router.tsx`, OUTSIDE WorkerLayout/RequireAuth — standalone like `do-scan`/`rack-scan`): piece summary + natural-sorted rack dropdown + two-tap Save, plain fetch + `csrfHeaders()`.
- **Sticker QR change:** `production/index.tsx` now enriches the FG sticker set with tokens (one batched authed call in `loadFgStickers` / the Foam pre-print path) and the 4 inline QR sites build their URL via `packingStickerUrl(s)` → `/p/<token>` when a token resolved, else the existing `/worker/scan` deep link (so the logged-in worker scan + completion flow is UNAFFECTED).
- **Rate limit:** added `/api/public/rack-write` to `api-rate-limit-config.ts` OVERRIDES at `{30/min, 300/hr}` (the tightened public ceiling), and brought the previously-missing `/api/public/rack-qr` under the same ceiling (it was falling back to DEFAULT 600/min).

**Regression test:** `tests/sticker-rack-public.test.mjs` (14 tests) — 64-hex token validation on both handlers, single-field scope-limited write, PACKING-only + rack-must-exist + empty-clears + PO-mirror in the shared helper, both-paths-use-shared-helper (no drift), card-from-token-row tenant safety, no-PII GET payload, authed-only minting, public prefix + tightened rate limit, unguessable token shape, snake_case `qr_token` column hygiene, token-never-logged. Updated the `security-public-endpoints` snapshot (`EXPECTED_PREFIXES`). Full suite: **1106 pass / 1 skip / 0 fail**. build:strict clean (only the 3 known jsbarcode/@zxing sandbox errors).

**Verify live:** print a Packing sticker (this mints the token). On a phone NOT logged into the Worker Portal, scan the sticker QR → lands on `/p/<token>` (no login) showing the piece + rack picker → pick a rack → two-tap Save → confirm `job_cards.rackingNumber` + `production_orders.rackingNumber` updated and the rack surfaces on the Packing sheet / DO. An invalid/garbled token (`/p/abc`) is rejected with a 404 "Unknown or expired QR code". The logged-in `/worker/scan` completion flow still works (any sticker whose token didn't mint keeps the `/worker/scan` fallback).
...
**Symptom:** on the default `/production` view (the all-orders Overview, header reads e.g. "6 of 1608 orders"), the operator types a customer PO that belongs to a COMPLETED/TRANSFERRED order. The count line updates ("6 of 1608"), but only some of the matched rows actually render (e.g. the 2 non-completed ones) — the fully-completed matches stay hidden. v1/v2/v3 of BUG-2026-06-23-004 (the `forceShowKeys` work) had already made the **per-dept tab** Production Sheet reveal searched completed rows, so "the dept tab works, the main view doesn't."

**Root cause (a DIFFERENT render path than v3 touched):** the DEFAULT `/production` view is `activeTab === "ALL"` — a **hand-rolled `<div>` Overview matrix** (`src/pages/production/index.tsx` ~L7437), NOT the dept `<DataGrid>` (~L6979) that v3 wired `forceShowKeys` onto. The Overview body renders `visibleOrders` (~L2854), which layers the Overview's OWN per-dept column filters on top of `filteredOrders`: a **dept-status value-filter** (`f.deptStatuses`, ~L2900) and a **dept-date filter** (`f.deptDates`, ~L2910), both **persisted in localStorage** (`OVERVIEW_TABLE_LS_KEY`). An operator who once set a dept column to show only Pending/Overdue (excluding Done) keeps that filter forever; a fully-"done" matched row is dropped by `if (!wanted.includes(c.state)) return false`. This Overview filter had **NO search exemption** (unlike the dept grid's `forceShowKeys` and unlike `filteredOrders`' own date-window `!q` skip), and the count line (~L6849) reads `filteredOrders.length` (which is NOT subject to the Overview column filter) → "6 of 1608" counted but only the non-completed rows rendered. v3 could never fix this: the Overview mounts no `<DataGrid>`, uses no `deptRows`/`deptForceShowKeys`, and hides COMPLETED via its own filter pipeline — not `defaultExcludedValues`.

**Fix (uniform principle — "skip the hide while searching, exempt matched rows" applied in EACH grid's own filter layer):** in the `visibleOrders` memo, while a top-bar search is active, BYPASS the Overview's `f.deptStatuses` AND `f.deptDates` filter loops (`src/pages/production/index.tsx` ~L2924, gated by `if (!searchActiveOverview)`). `searchActiveOverview` is derived from the **deferred** search (`deferredFltSearch`, not the urgent `searchActive`) so the bypass flips in lockstep with `filteredOrders` (which also reads `deferredFltSearch`) — gating on the urgent flag would, for one render, bypass the filters before `filteredOrders` had narrowed to the match, flashing every completed order. The page-level search/customer/state/category/date predicates already ran in `filteredOrders`, so a search that matches NOTHING leaves these rows empty and reveals no completed orders; a search that matches a completed order now renders it. With NO search active the two loops run exactly as before (byte-identical), so completed orders stay hidden on the default landing. After this fix `visibleOrders == filteredOrders` for searched completed rows, so the "X of N" count and the body agree — no count-line change needed. The dept grid was already covered by v3's `forceShowKeys`, so both grids now behave consistently. No grid remount (the Overview is a `<div>` matrix; multi-select state is untouched).

**Regression test:** extended `tests/production-batch-completion-reveal.test.mjs` (section 6) — asserts the `visibleOrders` memo derives `searchActiveOverview` from `deferredFltSearch`, gates BOTH the `f.deptStatuses` and `f.deptDates` loops behind `if (!searchActiveOverview)`, preserves the loop bodies unchanged (non-search byte-identical), and lists `deferredFltSearch` in its deps. The existing v3 dept-grid asserts are untouched. Full suite: 1080 pass / 1 skip / 0 fail. build:strict clean (only the 3 known jsbarcode/@zxing sandbox errors).
...
**REMEDIATION ROUND 2 (same day, owner: "SI 也 edit 還沒給錢的"):** swept ALL 186 unpaid (SENT, RM0-paid) invoices across every customer — invoice-level combo grouping (per SO-origin per baseModel) caught 3 stragglers the SO-scoped pass missed: INV-2605-089 (Houzs −RM243), INV-2605-118 (The Conts −RM73), INV-2605-017 (Carress −RM73) = −RM389. Invoice grand total: **22 invoices, −RM3,144.82**, 0 failures, 0 PAID touched.
...
## BUG-2026-08-06-005 — Voiding an EDITED sales invoice left its ledger legs live `accounting` `void-family`

🟢 Fixed. **(Dating correction, owner 2026-08-11: actually FOUND on 2026-08-11; the ID stays as minted since commits cite it.)** The void path reverses the visible family net (`LIKE 'invoice%'`, correct since BUG-2026-07-23-002) but then hid only the exact types `IN ('invoice','invoice_void')`. An edited invoice's live legs are `invoice_restate_post:<stamp>` — the hide left them visible while hiding the just-written reversal, so the cancelled invoice kept its full DR on 300-0000 and its CR on sales. Found live: the owner voided the two Carress error invoices (INV-2606-019 / 009, both restated on 2026-06-10) and **12,802.76 stayed on the debtor control** — surfacing as a **+12,802.76 half of the AR drift that partially cancelled the −25,000 Houzs half**, leaving the card at −12,197.24: two real errors reading as one smaller one. Fix = hide the whole `invoice%` family, org-agnostic (an orgId bind is exactly how BUG-2026-07-23-002 skipped five Carress cancels). Repair = re-run `backfill-cancel-reversals` after deploy. Regression: `tests/invoice-void-family.test.mjs`.

**Fourth instance of the void-family class** (BUG-2026-07-08-002 supplier payments · 2026-07-09 other-party bills · customer-payment lifecycle already used `payment%`). The rule: every module that edits via restate must treat the whole `<type>%` family as the document when voiding — reversal AND hide. When the class was fixed in July, the sweep stopped at the modules then known to restate; sales invoices gained restate later and inherited the hole.

## BUG-2026-08-06-004 — Editing a customer receipt never rebuilt the aging snapshot `accounting` `snapshot-staleness`

🟢 Fixed. **(Dating correction, owner 2026-08-11: actually FOUND on 2026-08-11; the ID stays as minted since commits cite it.)** The `/aging` snapshot rebuilds when a PROBED source table changes; it probes `kv_config`, not `payment_records`. A customer-payment restate rewrites `payment_records` and bumps invoice `paidAmount` without touching `updated_at` — no probed write, so the snapshot kept serving the old set. Found live: the owner re-allocated two Carress receipts (moving 14,418.00 off out-of-book invoices) and the aging kept reading **140,473.32** while `/ar-control`, the per-invoice recomputation and the counter all said **126,055.32**; a no-op `ar-include` re-flag (which bumps kv) refreshed it to exactly 126,055.32, proving staleness rather than a data fault. Fix = `bumpPaymentRecordsRev` (the customer twin of `bumpSupplierPaymentsRev`) pushed into **all four** mutation batches — create, status transition, lifecycle, restate. Regression: `tests/payment-aging-bump.test.mjs` counts bump-before-batch PAIRS, so a new mutation path cannot ship without it.
...
**Third instance of class C5 in one day, and the second I let through.** BUG-2026-08-06-001 (`purchase_invoice_unvoid`) shipped that morning with a regression test asserting "every correction sourceType resolves to a family" — but the test listed them **by hand**, so it passed while `labor_post` had been mis-dating every month-end posting all along. A hand-typed list can only cover the types its author already thought of, which is never the one that bites. The test now **scans `src/api/routes` for every `sourceType: "…"` the API writes** and fails on any that resolves to neither a family nor a self-dated sourceId, with an explicit exempt list that states each exemption's reason. `opening_balance` is exempt because it posts with an explicit `postedAt` of the opening date (verified on prod: all 274 legs sit on 2026-05-22 after three same-day re-posts) — it sets its date rather than escaping the rule.

## BUG-2026-08-06-002 — A forecast percentage divided by ACTUAL revenue `dashboard` `forecast`

🟢 Fixed. The Cost Structure card built its target share as `fc / sales` where `sales` is the ACTUAL revenue of the period. In a month only part-billed that reads a plan against a stub: 2026-08 showed **45,000 / 37,098 = 121.30%** for a target that is 15% of the 300,000 planned. Fix = a `csForecastSales(r)` base (`r.forecast.sales`, pro-rated by the actual line split in a bedframe/sofa view, exactly as `csForecastAmt` pro-rates the target itself), used by both the table and the Material Trend row beneath it. The actual share still divides by actual revenue — the point is that each side is a share of its OWN revenue, or comparing them says nothing. The Income Statement card already did this correctly (`share(pick(r.forecast), fcSales)`), which is what makes it a defect rather than a preference. Regression: `tests/dashboard-forecast-pct.test.mjs`.

**What kept it hidden.** The figure was equally wrong the day it shipped, but it lived *inside* the actual cell as a parenthetical `(fc 121.30%)` with no denominator in sight. The owner caught it within minutes of the forecast being split into its own column — because the 300,000 forecast revenue then sat two rows above the 121.30%, and the arithmetic became checkable at a glance. Lesson: a derived number displayed next to its own inputs gets audited by the reader; the same number displayed alone does not.
...
Owner spotted it on the Creditor page 2026-08-05 (「还有漂吗？」). Verified on prod: the card's −9,804.04 was **two** things, and this bug was 7,451.04 of it — `piOutstandingSen` exceeded both `/aging` and `/ap-reconciliation` by exactly Σ paid over the four part-paid bills (MEDITEX 3,374.40 + 1,754.40, OCEAN SKY 1,901.54, SUNMAT 420.70 = 7,451.04). After the fix all three endpoints agree to the cent (340,274.29). `/aging` and the ledger were correct throughout; `/ap-reconciliation` dual-keys correctly and never carried this error. Fix = dual-key the read + declare both spellings on the row type. Regression: `tests/ap-control-paid-read.test.mjs`. A repo-wide scan for other snake_case-only reads of `paid_amount_sen` found no second site.

**What the false alarm was hiding.** The remaining −2,353.00 is real and the reconciler names it: `payment_gl_mismatch` on **PV-2607-013** (MEDITEX) — RM 2,353.00 of that payment was booked against **PI-2605-030 (1,055.80) + PI-2605-031 (1,297.20)**, two invoices the owner deliberately EXCLUDED from the opening on 2026-07-02, so the payment debited 400-0000 where no liability was ever credited. Same shape as the GVP 950 case (2026-07-08). Owner's ruling to make, not a code defect — logged here only because a permanent false drift is what kept it invisible. **Beware measuring this from a browser: repeated GETs to the same API URL are served from the HTTP cache and will show pre-deploy numbers — verify with `cache: "no-store"` + a cache-buster, or a fix looks like it never shipped.**

**Class.** The camelCase-read bug (CLAUDE.md's standing rule: read rows dual-keyed). What made this instance survive months of use: the wrong number was *close* to the right one and lived beside a "drift" label that the reader is trained to expect a small number in. A field that silently reads `undefined` inside `|| 0` produces a plausible total, never an error.
