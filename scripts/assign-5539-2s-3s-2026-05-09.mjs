...
const COMPONENTS = ["2S", "3S"]; // 5539 master only has these (no 1S)
...
try {
  ...
  // 1. Pull 5539 product ids for 2S + 3S
  ...
  // 2. Pull 5531 customer prices for 2S + 3S as the source
  ...
  // 3. Step A: bulk-assign 5539-2S/3S to both customers
  console.log("\nStep A: bulk-assign");
  ...
  for (const cust of CUSTOMERS) {
    if (APPLY) {
      ...
      console.log(`  ${cust.label}: would bulk-assign ${productIds.length} (5539-2S, 5539-3S)`);
    }
  }

  // 4. Step B: POST customer prices for the 5539 cps using 5531 source
  console.log("\nStep B: customer prices (5531 → 5539 verbatim)");
  // After bulk-assign, fetch the cp ids
  let cpByKey = new Map();
  if (APPLY) {
    ...
    for (const comp of COMPONENTS) {
      ...
      if (APPLY) {
        ...
        const r = await authedJson(session, "POST", `/api/customer-products/${cpId}/prices`, {
          ...
          createdBy: "Wei Siang via assign-5539-2s-3s",
        });
