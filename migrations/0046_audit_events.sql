...
CREATE TABLE IF NOT EXISTS audit_events (
  ...
  actorUserId TEXT,                    -- nullable for system events
  actorUserName TEXT,                  -- snapshot of users.displayName
  actorRole TEXT,                      -- snapshot of role name (legacy users.role
  ...
  resourceId TEXT NOT NULL,            -- the row's PK in its own table
  ...
  ipAddress TEXT,                      -- if extractable from request headers
  userAgent TEXT,                      -- truncated UA string, ≤256 chars
  -- Time.
...
CREATE INDEX IF NOT EXISTS idx_audit_events_resource
  ON audit_events(resource, resourceId, ts DESC);

-- Lookup by actor (forensic queries: "what did user X touch on date Y")
CREATE INDEX IF NOT EXISTS idx_audit_events_actor
  ON audit_events(actorUserId, ts DESC);
