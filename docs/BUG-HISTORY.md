...
Owner spotted it on the Creditor page 2026-08-05 (「还有漂吗？」). Measured on prod twice, both exact: 4 part-paid bills → drift −9,804.04 = Σ paid; then 2 part-paid bills (SUNMAT K26050217 420.70 + OCEAN SKY 2605-327 1,901.54) → drift −2,322.24 = Σ paid. Nothing else was wrong: `/aging`, the ledger and `/ap-reconciliation` (which dual-keys correctly, `r.paidAmountSen ?? r.paid_amount_sen`, and reported drift 0.00 throughout) all read the paid amount properly — the defect was one handler's display sum, not the books. Fix = dual-key the read + declare both spellings on the row type. Regression: `tests/ap-control-paid-read.test.mjs`. A repo-wide scan for other snake_case-only reads of `paid_amount_sen` found no second site.
